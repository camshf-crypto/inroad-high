...
${sourceText ? `[참고용 - 생기부 활동 원문 (꼬리질문에 직접 인용 금지!)]\n${sourceText}\n[주의: 이건 단순 참고용. 학생이 답변에서 언급하지 않은 단어는 꼬리질문에 절대 사용하지 말 것]\n` : ''}
...
[꼬리질문 생성 규칙 - 매우 중요!!]
1. ★★★ 반드시 정확히 3개만 생성 ★★★ (4개 5개 절대 안됨, 1개 2개도 안됨)
...
[★★★ 절대 금지 - 매우 중요 ★★★]
1. **학생 답변에 없는 단어/개념을 사용하지 말 것**
   - 학생이 "지속 가능한 경영"이라고만 말했으면 → 절대 "생체모방", "ESG", "AI" 같은 단어 추가하지 말 것
   - 활동 원문에 있어도, 학생 답변에 없으면 절대 언급 X
   - 이유: 학생이 모르는 개념을 면접관이 물으면 학생만 곤란함
   
2. **꼬리질문은 무조건 학생 답변 안에 있는 단어/개념을 기반으로 만들 것**
   - 학생이 말한 "동아리 활동" → 그 동아리 활동에 대해 더 깊게 묻기
   - 학생이 말한 "발표" → 그 발표에 대해 구체적으로 묻기
   
3. 너무 일반적인 질문 X ("어려웠던 점은?")
4. 학생이 답변에서 이미 명확히 말한 부분 다시 묻기 X
5. 답변 불가능한 추상적 질문 X
6. 학생 답변에 없는 전문 용어/개념 추가 X (환각 방지)
...
JSON으로만 응답:
{
  "followups": [
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'system', 
            content: '너는 한국 대학 입시 면접관 출신 컨설턴트야. 학생 답변에서 진정성을 검증하고 파고드는 꼬리질문을 만들어. JSON만 반환해. 반드시 정확히 3개의 꼬리질문만 생성. 4개 5개 절대 안됨. ★중요: 학생 답변에 없는 단어/개념을 절대 추가하지 말 것. 학생이 "동아리"만 말했으면 "동아리"로만 질문, "생체모방", "AI" 같은 단어 절대 X.' 
          },
          { role: 'user', content: prompt },
        ],
        response_format: { type: 'json_object' },
        temperature: 0.3,
        max_tokens: 600,  // 3개 꼬리질문에 충분, 그 이상 만들 토큰 없음
      }),
    })
    ...
    // ★ 안전장치: AI가 5개 만들어도 강제로 3개로 자르기 (토큰 낭비 방지 + UI 일관성)
