...
function buildPrompt(
  ...
  type: string,
  grade: string,
  evalCriteria?: { name: string; score: number }[],
  semester?: string,
  topic?: string
): string {
  ...
  const topicNote = topic ? `\n\n[선생님이 지정한 수행평가 주제]\n"${topic}"\n반드시 이 주제를 중심으로 문제를 만들어줘. 주제에서 벗어나지 말 것.` : ''
...
- 전공 연계 가능한 주제 권장
- 세특 기재를 고려한 심화 탐구 활동${criteriaNote}${semesterNote}${topicNote}
...
- 다른 도서나 학습 내용과 연결하는 비교 독서 권장${criteriaNote}${semesterNote}${topicNote}
...
- 5~7분 발표 기준${criteriaNote}${semesterNote}${topicNote}
...
- 창의성·실현 가능성·완성도를 평가${criteriaNote}${semesterNote}${topicNote}
...
- 실생활 응용 사례 연결${criteriaNote}${semesterNote}${topicNote}
...
- 대입 수시 논술 수준의 사고력 요구${criteriaNote}${semesterNote}${topicNote}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { subject, type, grade, evalCriteria, semester, topic } = body
    ...
    const prompt = buildPrompt(subject, type, gradeLabel, evalCriteria, semester, topic)
