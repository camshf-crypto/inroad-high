...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const {
      ...
      activityPlan, // 🆕 활동 계획 (book에서 사용, topic도 받을 수 있음)
      author,       // 책 저자 (book일 때)
      // cell 모드 데이터
...
${activityPlan ? `[활동 계획] ${activityPlan}` : ''}
...
4. 구체적 활동 내용 + 깊이 있는 학습 과정 + 결과/통찰
5. 분량: 250~400자 (3~5문장)
6. 학과 연관성 자연스럽게 포함 (있을 때만)
7. 활동 계획이 있으면 그 방향으로 탐구가 진행될 것을 자연스럽게 반영
8. 추상적 표현 X, 구체적 표현 O

[출력]
세특 문장만 반환. 다른 설명 X. 마크다운 X. 
예시: "○○ 교과 탐구활동에서 '~~'을 주제로 심화 탐구를 진행함. ~~ 자료를 분석하고 ~~한 방법으로 접근하여 ~~한 결과를 도출함. 이 과정에서 ~~한 역량을 보였으며, ~~에 대한 깊이 있는 이해를 보여줌."`
      } else {
        // book
...
5. 분량: 60~120자 (1~2문장)
6. 학과 연관성 자연스럽게 포함 (있을 때만)
7. 활동 계획이 있으면 그 방향으로 사고가 확장됨을 자연스럽게 반영
8. 추상적 표현 X, 구체적 표현 O
