// supabase/functions/basic-analyze/index.ts
// ===============================================
// 💎 기본 인성질문 답변 분석
// 2가지 분석: ① 스피치 구조 충실도  ② 진로 컨셉(진로계열검사) 일치 검증
// 학교별 평가기준 없음 (인성질문은 학교 무관)
// ===============================================
...
- 구조의 각 요소가 답변에 들어있는지 하나하나 확인
- 빠진 요소, 순서가 어긋난 부분을 구체적으로 지적
- 구조에 맞게 보완하려면 어떻게 해야 하는지 실행 가능한 조언` : `정해진 스피치 구조 데이터가 없으므로, 일반적인 인성 면접 답변의 논리 구조(두괄식, 구체적 사례, 느낀 점/포부)를 기준으로 분석하세요.`}
...

{
  ...
  "tailSuggestions": ["꼬리질문1 (한글)", "꼬리질문2 (한글)", "꼬리질문3 (한글)", "꼬리질문4 (한글)"],
...
- JSON 외 다른 텍스트 절대 금지`
    ...
    console.error('basic-analyze error:', error)
    return new Response(
      JSON.stringify({
        success: false,
