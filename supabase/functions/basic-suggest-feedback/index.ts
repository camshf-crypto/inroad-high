// supabase/functions/basic-suggest-feedback/index.ts
// ===============================================
// 💎 기본 인성질문 - 선생님 말투 피드백 초안 생성
// 스피치 구조 분석 + 진로 컨셉 검증 결과를 바탕으로
// 학생에게 전달할 따뜻한 코치 말투 피드백을 작성
// ===============================================
...
interface RequestBody {
  ...
  secondAnswer?: string
  aiAnalysis?: any      // structureCheck / conceptCheck / summary 등
...
`.trim()
    ...
    const systemPrompt = `당신은 학생을 따뜻하게 지도하는 면접 스피치 코치 선생님입니다.
...
2. "기본 인성" 또는 "공통"을 학교 이름처럼 부르지 마세요. 이것은 학교가 아니라 인성면접 질문 카테고리입니다.
3. 학생에게 말하듯 친근하고 따뜻한 반말 코치 말투 (예: "~해보자", "~하면 좋겠어", "정말 잘했어")
...
🎯 진로 연결 코칭
(학생의 진로 목표와 이 답변을 어떻게 연결하면 더 좋아지는지 구체적으로 안내. 진로 컨셉이 답변에 잘 녹아있는지, 어떻게 보완할지. 3~4문장)
...
(학생의 노력을 진심으로 칭찬하고, 다음 답변을 기대한다는 따뜻한 한두 문장)`
    ...
    console.error('basic-suggest-feedback error:', error)
    return new Response(
      JSON.stringify({
        success: false,
