...
export default function MiddlePresentation() {
  ...
  const { data: curProblems = [] } = useSchoolYearProblems(
    selSchool || undefined,
    selYear || undefined,
  );
  const { data: questions = [] } = useProblemQuestions(
    selProblemId || undefined,
  );
  const { data: myAnswers = [] } = useMyPassageAnswers(
    studentId,
    selProblemId || undefined,
  );
  ...
  const feedbackByAnswerId = allFeedbacks.reduce(
  ...
  const [upgradedAnswers, setUpgradedAnswers] = useState<
  ...
  const getProblemStatus = (problemId: string) => {
    // 해당 문제의 질문들 가져와서 답변 상태 체크 — 사이드 호출 X (간단하게)
    // 여기서는 questions 배열에 의존하므로 list 화면에선 다른 방식 필요
    return "new";
  };
  ...
  const handleSubmitAnswer = async (questionId: string) => {
    ...
    try {
      ...
      setRecordings((p) => ({ ...p, [questionId]: false }));
    } catch (e: any) {
  ...
  const handleSubmitUpgrade = async (questionId: string) => {
    ...
    try {
      ...
      setRecordings((p) => ({ ...p, [`${questionId}-up`]: false }));
    } catch (e: any) {
      alert(`제출 실패: ${e.message}`);
    }
  };
  ...
  const handleSubmitTailAnswer = async (
    ...
    try {
      ...
      setRecordings((p) => ({ ...p, [tailKey]: false }));
    } catch (e: any) {
      alert(`제출 실패: ${e.message}`);
    }
  };
  ...
  if (step === "list") {
    ...
    return (
      <div className="h-full overflow-y-auto px-8 py-7 font-sans text-ink">
        ...
        <div className="text-[13px] text-ink-secondary mb-3.5">
          총 {curProblems.length}문제
        </div>
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          >
            <div className="text-[11px] font-bold text-ink-muted mb-2.5 tracking-wider uppercase">
              제시문
            </div>
            {selProblem.passage_pdf_url ? (
              <embed
                src={selProblem.passage_pdf_url}
                ...
                <div
                  ...
                  className={
                    qi < questions.length - 1
                      ? "border-b border-line-light pb-8"
                      : ""
                  }
                >
                  {/* 질문 */}
                  <div className="bg-brand-middle-pale border border-brand-middle-light rounded-xl px-4 py-3 mb-2.5">
                    <div className="text-[11px] font-bold text-brand-middle-dark mb-1">
                      문제 {qi + 1}.
                    </div>
                    <div className="text-[13px] font-semibold text-ink leading-[1.7]">
                      {q.text}
                    </div>
                  </div>
                      ...
                      return (
                        ...
                        >
                          {i < STEP_LABELS.length - 1 && (
                            <div
                              className={`absolute top-[11px] left-[55%] w-[90%] h-0.5 z-0 ${isDone ? "bg-brand-middle" : "bg-line"}`}
                            />
                          )}
                          <div
                            className={`w-[22px] h-[22px] rounded-full flex items-center justify-center text-[10px] font-bold z-10 relative border-2 ${
                              isDone || isOn
                  ...
                  {q.intent && q.intent.length > 0 && (
                    <div className="mb-3">
                      <button
                        onClick={() =>
                          setOpenIntents((p) => ({ ...p, [qKey]: !p[qKey] }))
                        }
                      ...
                      >
                        <span
                          className={`text-[11px] font-semibold ${openIntents[qKey] ? "text-amber-700" : "text-ink-secondary"}`}
                        >
                          💡 질문 의도 파악
                        </span>
                        ...
                        <div className="bg-amber-50/50 border border-amber-200 border-t-0 rounded-b-lg px-3 py-2.5">
                          <ul className="pl-4 space-y-1">
                            {q.intent.map((item: string, idx: number) => (
                              <li
                                key={idx}
                                className="text-[12px] text-amber-700 leading-[1.7] list-disc"
                              >
                                {item}
                              </li>
                            ))}
                  ...
                  <div className="bg-white border border-line rounded-xl px-4 py-3 mb-2.5">
                    <div className="flex items-center gap-1.5 mb-2">
                      <span
                        className={`text-[10px] font-bold text-white px-2 py-0.5 rounded-full ${qStep > 1 ? "bg-brand-middle" : "bg-ink-muted"}`}
                      >
                        Step 1
                      </span>
                    ...
                    {ans?.answer && !editingStep1[qKey] ? (
                      ...
                      <>
                        ...
                        <div className="flex gap-2 mt-2 justify-end">
                          ...
                          <MicBtn
                            recording={recordings[qKey] || false}
                            onClick={() =>
                              setRecordings((p) => ({ ...p, [qKey]: !p[qKey] }))
                            }
                          />
                          <SubmitBtn
                            label={
                  ...
                  {fb?.teacher_first_feedback && (
                    <div className="bg-white border border-line rounded-xl px-4 py-3 mb-2.5">
                      <div className="flex items-center gap-1.5 mb-2">
                        <span
                          className={`text-[10px] font-bold text-white px-2 py-0.5 rounded-full ${qStep > 3 ? "bg-brand-middle" : "bg-ink-muted"}`}
                        >
                          Step 3
                        </span>
                        <span className="text-[11px] text-ink-secondary font-medium">
                      ...
                      {ans?.upgraded_answer && !editingStep3[qKey] ? (
                        ...
                        <>
                          ...
                          <div className="flex gap-2 mt-2 justify-end">
                            ...
                            <MicBtn
                              recording={recordings[`${qKey}-up`] || false}
                              onClick={() =>
                                setRecordings((p) => ({
                                  ...p,
                                  [`${qKey}-up`]: !p[`${qKey}-up`],
                        ...
                        {fb.tail_questions.map((tail: any, ti: number) => {
                          ...
                          return (
                            <div
                              key={ti}
                              className={
                                ti < fb.tail_questions.length - 1 ? "mb-4" : ""
                              }
                            >
                              ...
                              {tail.answer ? (
                                ...
                                <>
                                  ...
                                  <div className="bg-gray-50 rounded-lg px-3 py-2.5">
                                    ...
                                    <div className="flex gap-2 mt-2 justify-end">
                                      <MicBtn
                                        recording={recordings[tailKey] || false}
                                        onClick={() =>
                                          setRecordings((p) => ({
                                            ...p,
                                            [tailKey]: !p[tailKey],
                                      ...
                                      <button
                                        onClick={() =>
                                          handleSubmitTailAnswer(q.id, ti)
                                        }
                                        disabled={
                                          !(
                                            tailAnswers[tailKey] || ""
                                          ).trim() || submitTailAnswer.isPending
                                        }
...
function ProblemCard({
  ...
  const status =
    answeredCount === 0 ? "new" : answeredCount === totalQ ? "done" : "doing";
