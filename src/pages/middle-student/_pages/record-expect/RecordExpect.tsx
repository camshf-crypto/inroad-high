// src/pages/middle-student/_pages/record-expect/RecordExpect.tsx
// 🌱 중등 생기부 예상질문 (고등 Expect.tsx 중등 버전)
...
import {
  ...
  getStep,
  type ExpectQuestion,
} from '../../_hooks/useMyMiddleSaenggibuQuestions'
...
const ALL_DEPTS = [
  ...
  '자유전공학부', '전기공학과', '전자공학과', '정치외교학과', '정치학과',
  '지구환경과학과', '지리학과', '지적재산학과', '철학과', '체육학과', '치의학과', '치위생학과',
  '컴퓨터공학과', '컴퓨터과학과', '통계학과', '특수교육학과', '한국어교육학과',
  '한문학과', '해양공학과', '행정학과', '화학공학과', '화학과', '환경공학과',
  '회계학과', '회화과', '물리치료학과', '방사선학과', '응급구조학과',
...
function DraftStatusIndicator({ status, lastSavedAt }: { status: DraftStatus; lastSavedAt: Date | null }) {
  ...
  if (status === 'error') return (
    <span className="text-[10px] text-red-500 flex items-center gap-1">⚠️ 저장 실패</span>
  )
  return (
    ...
    <button onClick={recording ? stopRecording : startRecording} disabled={processing}
      ...
      className={`w-9 h-9 rounded-lg border flex items-center justify-center flex-shrink-0 text-base transition-all ${
        recording ? 'bg-red-50 border-red-200 text-red-500 hover:bg-red-100 animate-pulse'
          ...
          : 'bg-brand-middle-pale border-brand-middle-light text-brand-middle-dark hover:bg-brand-middle-pale'
      }`}>
...
// ─────────────────────────────────────────────
// Step1 답변 박스 (자동저장 + STT)
// ─────────────────────────────────────────────
function Step1AnswerBox({ studentId, questionId, existingAnswer, editingMode, onSubmit, onCancel, isPending }: {
  ...
  const { text, setText, status, lastSavedAt, clearDraft } = useDraftAutoSave(
    studentId, `middle-record-expect:answer:${questionId}`, initialValue, disabled
  )
  return (
    <>
      <textarea value={text} onChange={e => setText(e.target.value)}
        ...
        rows={4}
        className="w-full border border-line rounded-lg px-3 py-2.5 text-[13px] outline-none resize-y leading-relaxed focus:border-brand-middle transition-colors font-sans" />
      <div className="flex items-center mt-2">
        <DraftStatusIndicator status={status} lastSavedAt={lastSavedAt} />
        <div className="flex gap-2 ml-auto items-center">
          {editingMode && (
            <button onClick={onCancel} className="h-9 px-3 bg-white text-ink-secondary border border-line rounded-lg text-[12px] font-semibold hover:bg-gray-50 transition-all">취소</button>
          )}
          ...
          <button onClick={() => onSubmit(text, clearDraft)} disabled={!text.trim() || isPending}
            className={`w-[108px] h-9 rounded-lg text-[12px] font-bold flex-shrink-0 transition-all ${
              text.trim() && !isPending
                ? 'bg-brand-middle text-white hover:bg-brand-middle-dark shadow-[0_2px_8px_rgba(16,185,129,0.2)]'
                : 'bg-gray-200 text-ink-muted cursor-not-allowed'
            }`}>
            {isPending ? '제출중...' : editingMode ? '수정 완료' : '답변 제출'}
          </button>
        </div>
      </div>
    </>
...
// Step3 업그레이드 박스
// ─────────────────────────────────────────────
function Step3UpgradeBox({ studentId, questionId, existingUpgrade, editingMode, onSubmit, onCancel, isPending }: {
  studentId: string | undefined
  questionId: string
  ...
  const { text, setText, status, lastSavedAt, clearDraft } = useDraftAutoSave(
    studentId, `middle-record-expect:upgraded:${questionId}`, initialValue, disabled
  )
  return (
    <>
      ...
      <div className="flex items-center mt-2">
        ...
        <div className="flex gap-2 ml-auto items-center">
          ...
          <button onClick={() => onSubmit(text, clearDraft)} disabled={!text.trim() || isPending}
            ...
            {isPending ? '제출중...' : editingMode ? '수정 완료' : '업그레이드 제출'}
          </button>
        </div>
      </div>
    </>
...
function Step5TailBox({ studentId, questionId, tailIndex, onSubmit, isPending }: {
  ...
  const { text, setText, status, lastSavedAt, clearDraft } = useDraftAutoSave(
    studentId, `middle-record-expect:tail:${questionId}:${tailIndex}`, ''
  )
  return (
    <div className="bg-gray-50 rounded-lg px-3 py-2.5">
      <textarea value={text} onChange={e => setText(e.target.value)}
        ...
        rows={3}
        className="w-full border border-line rounded-lg px-2.5 py-2 text-[12px] outline-none resize-y leading-relaxed bg-white focus:border-brand-middle transition-colors font-sans" />
      <div className="flex items-center mt-2">
        <DraftStatusIndicator status={status} lastSavedAt={lastSavedAt} />
        <div className="flex gap-2 ml-auto items-center">
          <MicSTTBtn onTranscript={t => setText(text ? text + ' ' + t : t)} />
...
// 메인
// ─────────────────────────────────────────────
export default function RecordExpect() {
  ...
  const [deptSearch, setDeptSearch] = useState<Record<string, string>>({ '중1': '', '중2': '', '중3': '' })
  const [selDept, setSelDept] = useState<Record<string, string>>({ '중1': '', '중2': '', '중3': '' })
  const [deptDropOpen, setDeptDropOpen] = useState<Record<string, boolean>>({ '중1': false, '중2': false, '중3': false })
  ...
  useEffect(() => {
    if (uploadedPdf) {
      ...
      if (recentDept) setSelDept(prev => ({ ...prev, [grade]: recentDept }))
    }
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return
    if (!selDept[grade]) { alert('관심 학과를 먼저 선택해주세요!'); return }
    if (file.size > 50 * 1024 * 1024) { alert('파일 크기는 50MB 이하로 올려주세요.'); return }
    uploadPdf.mutate({ file, grade: gradeNum, majorDept: selDept[grade] }, {
      onSuccess: () => { alert('생기부 업로드 완료! 선생님이 예상질문을 만들어줄 거에요.'); refetchPdf() },
      onError: (err: any) => { alert('업로드 실패: ' + (err?.message || '알 수 없는 오류')) },
    })
  }
  ...
  const handleDeleteFile = () => {
    ...
    deletePdf.mutate(uploadedPdf.path, { onSuccess: () => refetchPdf() })
  }
  ...
  const handleSubmitUpgraded = async (text: string, clearDraft: () => Promise<void>) => {
    if (!text.trim() || !selQ) return
    submitUpgraded.mutate({ questionId: selQ.id, answer: text.trim() }, {
      onSuccess: async () => { await clearDraft(); setEditingStep3(false) },
    })
  }

  const handleSubmitTail = async (followupId: string, text: string, clearDraft: () => Promise<void>) => {
    if (!text.trim()) return
          ...
          {['중1', '중2', '중3'].map(g => (
            <button key={g} onClick={() => setGrade(g)}
              className={`px-4 py-1.5 rounded-full text-[13px] border transition-all ${
                grade === g
                  ? 'bg-brand-middle text-white border-brand-middle font-semibold shadow-[0_2px_8px_rgba(16,185,129,0.15)]'
                  : 'bg-white text-ink-secondary border-line hover:border-brand-middle-light hover:text-brand-middle-dark font-medium'
              }`}>
              {g}
            </button>
          ))}
        ...
        <div className="w-[360px] flex-shrink-0 bg-white border border-line rounded-2xl flex flex-col overflow-hidden shadow-[0_4px_16px_rgba(15,23,42,0.04)]">
            ...
            {[{ key: 'questions', label: '예상질문' }, { key: 'upload', label: '생기부 업로드' }].map(t => (
              <button key={t.key} onClick={() => setActiveTab(t.key as any)}
                className={`flex-1 py-3 text-[13px] font-semibold transition-colors border-b-2 ${
                  activeTab === t.key ? 'text-brand-middle-dark border-brand-middle' : 'text-ink-secondary border-transparent hover:text-ink'
                }`}>
                {t.label}
              </button>
            ))}
            ...
            <>
              <div className="px-4 py-2.5 border-b border-line-light flex-shrink-0">
                <div className="text-[12px] text-ink-secondary font-medium leading-[1.7]">
                  총 <span className="text-brand-middle-dark font-bold">{questions.length}개</span><br />
              ...
              <div className="flex-1 overflow-y-auto p-3">
                {loadingQ ? (
                  ...
                  <div className="text-center py-10 text-ink-muted">
                    ...
                    <div className="text-[11px] leading-relaxed">생기부를 업로드하면<br />선생님이 예상질문을 만들어줘요.</div>
                  </div>
                ) : questions.map((q, i) => {
                  ...
                  return (
                    <div key={q.id} onClick={() => setSelQId(q.id)}
                      ...
                      <span className="text-[10px] font-bold text-brand-middle-dark bg-brand-middle-pale px-2 py-0.5 rounded-full inline-block mb-1.5">질문 {i + 1}</span>
                      <div className="text-[12.5px] text-ink leading-relaxed font-semibold mb-1.5">{q.teacher_edited_question || q.question}</div>
                      <div className="flex gap-1 flex-wrap">
                        {q.tag && <span className="text-[10px] font-medium text-ink-secondary bg-gray-100 px-2 py-0.5 rounded-full">{q.tag}</span>}
                        {hasAnswer
          ...
          {activeTab === 'upload' && (
            <div className="flex-1 overflow-y-auto p-4">
              <div className="text-[11px] font-semibold text-ink-secondary bg-gray-100 px-3 py-1 rounded-full inline-block mb-3">{grade} 생기부</div>
              <div className="mb-4">
                <div className="text-[13px] font-bold text-ink mb-1">관심 학과 선택</div>
                <div className="text-[12px] text-ink-secondary mb-2 leading-relaxed">관심 있는 학과를 선택하면 더 맞춤형 예상질문을 만들어 드려요.</div>
                <div className="relative">
                  <div onClick={() => setDeptDropOpen(prev => ({ ...prev, [grade]: true }))}
                    className={`flex items-center gap-2 border rounded-lg px-3 py-2.5 bg-white cursor-text transition-colors ${deptDropOpen[grade] ? 'border-brand-middle' : 'border-line'}`}>
                    <span className="text-ink-muted">🔍</span>
                    <input
                      value={selDept[grade] && !deptDropOpen[grade] ? selDept[grade] : deptSearch[grade]}
                      onChange={e => { setDeptSearch(prev => ({ ...prev, [grade]: e.target.value })); setSelDept(prev => ({ ...prev, [grade]: '' })); setDeptDropOpen(prev => ({ ...prev, [grade]: true })) }}
                    ...
                    {selDept[grade] && (
                      <button onClick={e => { e.stopPropagation(); setSelDept(prev => ({ ...prev, [grade]: '' })); setDeptSearch(prev => ({ ...prev, [grade]: '' })) }}
                        className="text-[11px] text-ink-muted hover:text-ink">✕</button>
                    )}
                  </div>
                  {deptDropOpen[grade] && (
                    <>
                      <div onClick={() => setDeptDropOpen(prev => ({ ...prev, [grade]: false }))} className="fixed inset-0 z-10" />
                      <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-line rounded-lg z-20 max-h-[200px] overflow-y-auto shadow-lg">
                        {filteredDepts.length === 0
                          ...
                          : filteredDepts.map((d, i) => (
                            <div key={i}
                              onClick={() => { setSelDept(prev => ({ ...prev, [grade]: d })); setDeptSearch(prev => ({ ...prev, [grade]: '' })); setDeptDropOpen(prev => ({ ...prev, [grade]: false })) }}
                              className={`px-3 py-2 text-[12px] cursor-pointer transition-colors ${selDept[grade] === d ? 'bg-brand-middle-pale text-brand-middle-dark font-semibold' : 'text-ink hover:bg-brand-middle-pale/50'} ${i < filteredDepts.length - 1 ? 'border-b border-line-light' : ''}`}>
                              {d}
                            </div>
                          ))}
                      </div>
                ...
                {selDept[grade] && (
                  <div className="mt-2 flex items-center gap-1.5 bg-brand-middle-pale border border-brand-middle-light rounded-lg px-3 py-2">
                    <span className="text-brand-middle-dark font-bold">✓</span>
                    <span className="text-[12px] text-brand-middle-dark font-semibold">{selDept[grade]}</span>
              ...
              <div>
                ...
                <div className="text-[12px] text-ink-secondary mb-3 leading-relaxed">생기부를 업로드하면 선생님이 예상질문을 생성해드려요.</div>
                <input type="file" accept=".pdf" id={`upload-${grade}`} className="hidden" onChange={handleFileSelect} disabled={uploadPdf.isPending} />
                {uploadedPdf ? (
                  <>
                    <div className="border border-emerald-300 rounded-xl p-4 bg-emerald-50 flex items-center gap-3 mb-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[11px] text-emerald-600 font-medium">{(uploadedPdf.size / 1024).toFixed(1)} KB · 업로드 완료</div>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <label htmlFor={`upload-${grade}`} className="flex-1 py-2 bg-white text-brand-middle-dark border border-brand-middle rounded-lg text-[12px] font-bold text-center cursor-pointer hover:bg-brand-middle-pale transition-all">🔄 다시 업로드</label>
                      <button onClick={handleDeleteFile} disabled={deletePdf.isPending}
                  ...
                  <div className="border-[1.5px] border-dashed border-brand-middle-light rounded-xl py-7 text-center bg-brand-middle-pale/30">
                    <div className="text-3xl mb-2">⏳</div>
                    <div className="text-[13px] font-bold text-brand-middle-dark">업로드 중...</div>
                  </div>
                ) : (
                  <label htmlFor={selDept[grade] ? `upload-${grade}` : undefined}
                    onClick={e => { if (!selDept[grade]) { e.preventDefault(); alert('먼저 관심 학과를 선택해주세요!') } }}
                    className={`block border-[1.5px] border-dashed rounded-xl py-7 text-center transition-all ${selDept[grade] ? 'border-brand-middle-light bg-brand-middle-pale/30 cursor-pointer hover:bg-brand-middle-pale' : 'border-line bg-gray-50 cursor-not-allowed'}`}>
                    <div className="text-3xl mb-2">📄</div>
                    <div className={`text-[13px] font-bold ${selDept[grade] ? 'text-brand-middle-dark' : 'text-ink-muted'}`}>PDF 파일 업로드</div>
                    <div className="text-[11px] text-ink-muted mt-1 font-medium">{selDept[grade] ? `${grade} 생기부를 업로드해주세요` : '관심 학과를 먼저 선택해주세요'}</div>
                  </label>
                )}
              </div>
            </div>
            ...
            <>
              <div className="px-5 py-3.5 border-b border-line-light flex-shrink-0">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`text-[11px] font-bold px-2.5 py-1 rounded-full border ${selQ.student_answer ? 'text-emerald-700 bg-emerald-50 border-emerald-200' : 'text-amber-700 bg-amber-50 border-amber-200'}`}>
                      ...
                      <div key={i} className="flex-1 flex flex-col items-center gap-1 relative">
                        ...
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold z-10 border-2 transition-all ${
                          isDone ? 'bg-emerald-500 text-white border-emerald-500'
                            : isOn ? 'bg-brand-middle text-white border-brand-middle shadow-[0_2px_8px_rgba(16,185,129,0.3)]'
                            : 'bg-gray-100 text-ink-muted border-line'
                        }`}>{isDone ? '✓' : stepNum}</div>
                        <div className={`text-[10px] font-semibold whitespace-nowrap ${isDone ? 'text-emerald-600' : isOn ? 'text-brand-middle-dark' : 'text-ink-muted'}`}>{label}</div>
                      </div>
                    )
                  })}
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 bg-gray-50">
                ...
                <div className="bg-white border border-line rounded-xl px-4 py-3">
                  ...
                  <div className="text-[14px] font-bold text-ink leading-relaxed">{selQ.teacher_edited_question || selQ.question}</div>
                </div>
                  ...
                  <div className="bg-brand-middle-pale border border-brand-middle-light rounded-xl px-4 py-3">
                    ...
                    <ul className="pl-4 m-0">
                      {selQ.purpose.map((p: string, i: number) => (
                        <li key={i} className="text-[12px] text-brand-middle-dark leading-relaxed list-disc">{p}</li>
                      ))}
                    </ul>
                  </div>
                )}
                ...
                <div className="bg-white border border-line rounded-xl px-4 py-3">
                  ...
                  {selQ.student_answer && !editingStep1 ? (
                    <div>
                      <div className="bg-gray-50 border border-line-light rounded-lg px-3 py-2.5 text-[13px] text-ink leading-relaxed mb-2 whitespace-pre-wrap">{selQ.student_answer}</div>
                      <div className="flex justify-end">
                        <button onClick={() => setEditingStep1(true)}
                          className="text-[11px] font-semibold text-ink-secondary bg-white border border-line rounded-md px-3 py-1 hover:bg-gray-50 transition-all">✏️ 수정</button>
                      </div>
                    </div>
                  ) : (
                    <Step1AnswerBox
                      ...
                      studentId={studentId}
                      questionId={selQ.id}
                      existingAnswer={selQ.student_answer ?? null}
                ...
                {selQ.student_answer && (
                  <div className="bg-white border border-line rounded-xl px-4 py-3">
                    ...
                    {round1?.teacher_feedback ? (
                      <div className="bg-brand-middle-pale border border-brand-middle-light rounded-lg px-3 py-2.5 text-[13px] text-brand-middle-dark leading-relaxed whitespace-pre-wrap">{round1.teacher_feedback}</div>
                    ) : (
                  ...
                  <div className="bg-white border border-line rounded-xl px-4 py-3">
                    ...
                    {round2?.revised_answer && !editingStep3 ? (
                      <div>
                        ...
                        <div className="flex justify-end">
                          <button onClick={() => setEditingStep3(true)}
                            className="text-[11px] font-semibold text-ink-secondary bg-white border border-line rounded-md px-3 py-1 hover:bg-gray-50 transition-all">✏️ 수정</button>
                        </div>
                      </div>
                    ) : (
                      <Step3UpgradeBox
                        ...
                        onSubmit={handleSubmitUpgraded}
                        onCancel={() => setEditingStep3(false)}
                        isPending={submitUpgraded.isPending}
                      />
                    )}
                  </div>
                )}
                    ...
                    {followups.map((fu, i) => (
                      <div key={fu.id} className="mb-3">
                        <div className="flex items-start gap-2 px-3 py-2 bg-gray-50 border border-line-light rounded-lg mb-2 text-[12px] text-ink leading-relaxed">
                          <span className="text-[10px] font-bold text-brand-middle-dark bg-brand-middle-pale px-2 py-0.5 rounded-full flex-shrink-0 mt-0.5">꼬리 {i + 1}</span>
                        ...
                        {fu.student_answer ? (
                          ...
                          <Step5TailBox
                            ...
                            questionId={selQ.id}
                            tailIndex={i}
                            onSubmit={(text: any, clearDraft: any) => handleSubmitTail(fu.id, text, clearDraft)}
                            isPending={submitTail.isPending}
                          />
                        )}
                      </div>
                    ))}
