...
function MicSTTBtn({ studentId, onTranscript }: MicSTTBtnProps) {
  ...
  const stopRecording = async () => {
    ...
    try {
      ...
      const fileName = `${studentId}/expect-${Date.now()}.webm`;
                      ...
                      <div className="bg-brand-middle-pale border border-brand-middle-light rounded-xl px-4 py-3">
                        ...
                        <ul className="pl-4">
                          {selQ.purpose.map((p: string, i: number) => (
                            <li key={i} className="text-[12px] text-brand-middle-dark leading-[1.7] list-disc">{p}</li>
                          ))}
                        </ul>
                      </div>
                    )}
                      ...
                      {selQ.answer && !editingStep1 ? (
                        ...
                        <>
                          ...
                          <div className="flex gap-2 mt-2 justify-end">
                            ...
                            />
                            <SubmitBtn
                              label={submitAnswer.isPending ? "제출 중..." : editingStep1 ? "수정 완료" : "답변 제출"}
                      ...
                      <div className="bg-white border border-line rounded-xl px-4 py-3">
                        ...
                        {selQ.upgraded_answer && !editingStep3 ? (
                          <div>
                            ...
                            <div className="flex justify-end">
                              <button
                                onClick={() => { setEditingStep3(true); setUpgradedAnswer(selQ.upgraded_answer || ""); }}
                                className="text-[11px] font-medium text-ink-secondary bg-white border border-line rounded-md px-2.5 py-1 hover:border-brand-middle-light hover:text-brand-middle-dark transition-all"
                              >
                                ✏️ 수정
                              </button>
                          ...
                          <>
                            ...
                            <div className="flex gap-2 mt-2 justify-end">
                              ...
                              <MicSTTBtn
                                studentId={studentId}
                                onTranscript={(text) => setUpgradedAnswer((prev) => prev ? prev + " " + text : text)}
                              />
                              <SubmitBtn
                                label={submitUpgrade.isPending ? "제출 중..." : editingStep3 ? "수정 완료" : "업그레이드 제출"}
                          ...
                          <div key={i} className="mb-3">
                            ...
                            {t.answer ? (
                              ...
                              <div className="bg-gray-50 rounded-lg px-3 py-2.5">
                                ...
                                <textarea
                                  value={tailAnswers[i] || ""}
                                  onChange={(e) => setTailAnswers((prev) => ({ ...prev, [i]: e.target.value }))}
                                ...
                                <div className="flex gap-2 mt-2 justify-end">
                                  ...
                                  <button
                                    ...
                                    disabled={!(tailAnswers[i] || "").trim() || submitTailAnswer.isPending}
                                    className={`w-[102px] h-[34px] rounded-md text-[12px] font-semibold transition-all ${(tailAnswers[i] || "").trim() && !submitTailAnswer.isPending
                                        ? "bg-brand-middle hover:bg-brand-middle-hover text-white hover:-translate-y-px hover:shadow-btn-middle"
                                        : "bg-gray-100 text-ink-muted cursor-not-allowed"
                                      }`}
                                  >
