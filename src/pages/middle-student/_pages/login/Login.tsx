...
import { useLogin, usePasswordReset } from '@/lib/auth/useLogin'
...
export default function MiddleLogin() {
  ...
  const { handleLogin, loading, error, setError } = useLogin({
    ...
    loginType: 'student',
    defaultGrade: '중1',
    roleErrorMessage: '중학생 계정만 접근 가능합니다.',
  })

  const { handleFindPw, findResult, findLoading, setFindResult } = usePasswordReset(
    '/middle-student/login'
  )
        ...
        <div className="flex-1 flex flex-col justify-center px-8">
          <div className="w-full max-w-[360px] mx-auto">
            ...
            <div className="mb-8 text-center">
              ...
              <p className="text-[13px] text-ink-secondary">중등 학생 계정으로 로그인하세요.</p>
            </div>
            ...
            <div className="flex items-center justify-center gap-4 text-[13px]">
              <button
                ...
                className="text-ink-secondary hover:text-brand-middle-dark transition-colors"
              >
                아이디 찾기
              </button>
              ...
              <span className="text-line">|</span>
              <button
                onClick={() => navigate('/middle-student/signup')}
                className="text-brand-middle-dark font-bold hover:text-brand-middle transition-colors"
              >
                회원가입
              </button>
            </div>
      ...
      <div className="flex-1 max-md:hidden flex flex-col items-center justify-center px-16 py-12 bg-gradient-to-br from-[#F0FDF4] to-[#F8FAFC] relative overflow-hidden">
        ...
        <div className="relative text-center mb-12 max-w-[580px]">
          ...
          <h1 className="text-[40px] max-md:text-3xl font-extrabold text-ink tracking-[-0.03em] leading-[1.2] mb-4">
            고입 합격,<br />
            <span className="bg-gradient-to-br from-brand-middle-dark to-brand-middle bg-clip-text text-transparent">
              지금 시작하세요
            </span>
          </h1>
        ...
        <div className="grid grid-cols-2 gap-4 w-full max-w-[520px] relative">
          {[
            ...
            { icon: '🎬', title: '수업 영상', desc: '강의 영상 + 교재 열람' },
            { icon: '📝', title: '숙제 관리', desc: '숙제 제출 + 선생님 피드백' },
            { icon: '🎙️', title: '면접 시뮬레이션', desc: '중3 고입 면접 실전 대비' },
          ].map((f, i) => (
            <div
              key={i}
              className="bg-white rounded-2xl px-5 py-4 border border-line hover:border-brand-middle-light hover:shadow-[0_8px_24px_rgba(16,185,129,0.08)] hover:-translate-y-0.5 transition-all"
            >
              <div className="w-10 h-10 bg-brand-middle-pale rounded-xl flex items-center justify-center text-xl mb-3 border border-brand-middle-light">
                {f.icon}
              </div>
            ...
            {modal === 'findEmail' && (
              <div>
                ...
                {findResult && (
                  <div className={`px-4 py-3 rounded-lg mb-4 text-[13px] border ${
                    findResult.startsWith('ok:')
                      ? 'bg-brand-middle-pale border-brand-middle-light text-brand-middle-dark font-semibold'
                ...
                <button
                  ...
                  className={`w-full h-11 rounded-lg text-[14px] font-semibold text-white transition-all bg-brand-middle hover:bg-brand-middle-hover hover:-translate-y-px hover:shadow-btn-middle`}
                >
                  아이디 찾기
                </button>
              </div>
              ...
              <div>
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onKeyDown={e => e.key === 'Enter' && handleFindPw(findEmail)}
                  />
                </div>
                ...
                <button
                  onClick={() => handleFindPw(findEmail)}
