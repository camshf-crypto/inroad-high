// src/pages/middle-student/_pages/concept/ConceptDiagnosis.tsx
...
export default function ConceptDiagnosis({ conceptData, studentId, academyId, grade, prevConcept, onComplete, onDirectSelect }: Props) {
  ...
  const saveProgress = async (updatedAnswers: Record<number, string>, qNum: number) => {
    ...
    try {
      ...
      if (existing) {
        await supabase.from('middle_student_concept').update(payload).eq('id', existing.id)
      } else {
        await supabase.from('middle_student_concept').insert(payload)
      }
    } catch (e) {
        ...
        <div className="max-w-[440px] w-full bg-white border border-line rounded-2xl p-5 shadow-[0_8px_32px_rgba(15,23,42,0.08)]">
          <div className="text-center mb-4">
            ...
            <div className="text-[12px] text-ink-secondary leading-relaxed">
              <span className="font-bold text-brand-middle-dark">Q.{String(conceptData?.current_question ?? 1).padStart(3, '0')}</span>까지 답변하셨어요
          ...
          <div className="mb-4">
            <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-brand-middle to-brand-middle-dark rounded-full transition-all"
      ...
      <div className="h-full overflow-y-auto flex items-center justify-center px-6 py-5 bg-[#F8FAFC]">
        <div className="max-w-[560px] w-full">
          <div className="text-center mb-4">
            ...
            <div className="text-[17px] font-extrabold text-ink tracking-tight mb-0.5">진로 계열 검사 설정</div>
          ...
          <div className="grid grid-cols-2 gap-3 mb-3">
            <button
              onClick={() => setStarted(true)}
              className="bg-white border-2 border-line rounded-xl p-4 text-left hover:border-brand-middle hover:shadow-[0_4px_20px_rgba(16,185,129,0.1)] transition-all hover:-translate-y-px group"
            >
              <div className="flex items-center gap-2 mb-2">
                <div className="text-xl">📝</div>
                <div className="text-[14px] font-extrabold text-ink group-hover:text-brand-middle-dark">계열 검사하기</div>
              </div>
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <div className="text-xl">🎯</div>
                <div className="text-[14px] font-extrabold text-ink group-hover:text-brand-middle-dark">직접 선택하기</div>
              </div>
              <div className="text-[11px] text-ink-secondary leading-relaxed mb-2.5">
                계열/학과/직업을 직접 선택해서 바로 진로 계열 검사를 설정해요
              </div>
              <div className="flex flex-col gap-0.5">
                <div className="text-[10.5px] text-ink-muted">⚡ 5분 이내 완료</div>
                <div className="text-[10.5px] text-ink-muted">🔄 언제든 수정 가능</div>
                {prevConcept?.major && <div className="text-[10.5px] text-brand-middle font-semibold">↩ 이전 컨셉 참고 추천 제공</div>}
              </div>
            </button>
          </div>
    ...
    <div className="h-full flex flex-col overflow-hidden bg-[#F8FAFC]">
      ...
      <div className="bg-white border-b border-line px-6 py-2 flex-shrink-0">
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-[11px] font-medium text-ink-secondary hover:text-brand-middle-dark transition-colors flex items-center gap-0.5 px-1.5 py-0.5 rounded hover:bg-gray-50"
              title="진로 계열 검사 처음 화면으로 (답변은 저장돼요)"
            >
              ← 처음으로
            </button>
            <div className="w-px h-3 bg-line" />
            <span className="text-[11px] font-bold text-brand-middle-dark bg-brand-middle-pale px-2 py-0.5 rounded-full border border-brand-middle-light">
              {currentPart?.title}
            </span>
        ...
        <div className="w-full h-1.5 bg-gray-100 rounded-full overflow-hidden">
          <div
            className="h-full bg-gradient-to-r from-brand-middle to-brand-middle-dark rounded-full transition-all duration-300"
            style={{ width: `${progress}%` }}
          />
        </div>
      </div>
        ...
        <div className="max-w-[700px] mx-auto px-6 py-6 w-full">
          ...
          <div className="mb-5">
            <div className="flex items-center gap-2 mb-3">
              <span className="w-8 h-8 rounded-lg bg-brand-middle text-white text-[13px] font-extrabold flex items-center justify-center flex-shrink-0 shadow-[0_2px_8px_rgba(16,185,129,0.25)]">
                {currentQ}
              </span>
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl border transition-all hover:-translate-y-px text-[13.5px] font-medium leading-relaxed ${
                    isSelected
                      ? 'border-brand-middle bg-brand-middle text-white shadow-[0_4px_12px_rgba(16,185,129,0.2)]'
                      : 'border-line bg-white text-ink hover:border-brand-middle-light hover:shadow-sm'
                  }`}
                >
                  {choice}
                </button>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-[11px] text-ink-muted">
              답변: <span className="font-bold text-brand-middle">{totalAnswered}</span> / 200
            </div>
            ...
            {currentQ < 200 ? (
              <button
                ...
                disabled={!isAnswered}
                className="px-4 py-2 bg-brand-middle text-white rounded-lg text-[12px] font-semibold hover:bg-brand-middle-dark transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
              >
                다음 →
              </button>
            ) : (
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-brand-middle-dark to-brand-middle text-white rounded-lg text-[12px] font-bold shadow-[0_2px_8px_rgba(16,185,129,0.25)] hover:opacity-90 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
              >
