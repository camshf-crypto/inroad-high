// src/pages/middle-student/_hooks/useMyMiddleSaenggibuQuestions.ts
// 🌱 중등 생기부 예상질문 시스템 hook (고등 hook의 중등 버전)
...
export function useMyQuestions(grade: number) {
  ...
  return useQuery({
    queryKey: ['my-middle-expect-questions', studentId, grade],
    enabled: !!studentId,
...
export function useMyQuestionAnalyses(questionId: string | undefined) {
  return useQuery({
    queryKey: ['my-middle-expect-analyses', questionId],
    enabled: !!questionId,
    refetchInterval: POLL_INTERVAL,
    refetchIntervalInBackground: false,
...
export function useMyQuestionFollowups(questionId: string | undefined) {
  return useQuery({
    queryKey: ['my-middle-expect-followups', questionId],
    enabled: !!questionId,
    refetchInterval: POLL_INTERVAL,
    refetchIntervalInBackground: false,
...
// ─────────────────────────────────────────────
// 4. PDF 업로드 (Supabase Storage + middle_saenggibu_uploads 테이블)
// ─────────────────────────────────────────────
...
export function useUploadSaenggibuPdf() {
  ...
  return useMutation({
    mutationFn: async ({ file, grade, majorDept }: {
      ...
      // 3. middle_saenggibu_uploads 테이블에 메타데이터 저장
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['my-middle-saenggibu-pdf'] })
    },
  })
}
...
export function useMyUploadedPdf(grade: number) {
  ...
  return useQuery({
    queryKey: ['my-middle-saenggibu-pdf', studentId, grade],
    queryFn: async () => {
      if (!studentId) return null

      // middle_saenggibu_uploads 테이블에서 메타데이터 조회
      ...
      return {
        path: uploadData?.pdf_path || `${folderPath}/${file.name}`,
        name: file.name,
...
export function useDeleteMyPdf() {
  ...
  return useMutation({
    mutationFn: async (path: string) => {
      ...
      // middle_saenggibu_uploads 테이블에서도 삭제
      const { error: dbError } = await supabase
        .from('middle_saenggibu_uploads')
        .delete()
        .eq('pdf_path', path)
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['my-middle-saenggibu-pdf', studentId] })
    },
  })
}
...
export function useSubmitFirstAnswer() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, v) => {
      qc.invalidateQueries({ queryKey: ['my-middle-expect-questions'] })
      qc.invalidateQueries({ queryKey: ['my-middle-expect-analyses', v.questionId] })
    },
  })
}
...
export function useSubmitFollowupAnswer() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['my-middle-expect-followups'] })
    },
  })
}
