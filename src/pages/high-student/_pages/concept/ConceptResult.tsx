...
export default function ConceptResult({ conceptData, onRefresh, prevConcept, isDirectMode = false, studentId, academyId }: Props) {
  ...
  }, [conceptData.id, conceptData.major, conceptData.grade])

  // ⭐ 학과 선택되면 그 학과의 키워드를 DB에서 로드
  useEffect(() => {
    if (!selectedMajor) {
      setKeywordOptions([])
      return
    ...
    const loadKeywords = async () => {
      ...
      try {
        ...
        // 2) 그 학과의 키워드 가져오기 (display_order 순)
        const { data: kws, error: kwErr } = await supabase
          .from('high_career_keyword')
          .select('keyword')
  ...
  return (
    <div className="h-full overflow-y-auto bg-[#F8FAFC]">
      <div className="max-w-[640px] mx-auto px-6 py-5">
        ...
        {/* 헤더 */}
        <div className="mb-4">
          <div className="text-[10px] font-bold text-brand-high-dark uppercase tracking-wider mb-1">진로 계열 검사 설정</div>
          ...
          {[
            ...
            return (
              <div key={s.key} className="flex items-center gap-1.5">
                <div className={`flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold transition-all ${step === s.key ? 'bg-brand-high text-white' : sIdx < curIdx ? 'bg-brand-high-pale text-brand-high-dark border border-brand-high-light' : 'bg-white text-ink-muted border border-line'
                  }`}>
                  ...
                  {recommendedMajors
                    ...
                    .map(m => (
                      <button key={m} onClick={() => setSelectedMajor(m)}
                        className={`text-[10.5px] font-semibold px-2.5 py-0.5 rounded-full border transition-all ${selectedMajor === m
              ...
              <div className="grid grid-cols-2 gap-1.5 mb-3">
                {recommendedMajors.map(major => (
                  <button key={major} onClick={() => setSelectedMajor(major)}
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-left text-[12px] font-medium transition-all hover:-translate-y-px ${selectedMajor === major ? 'border-brand-high bg-brand-high text-white' : 'border-brand-high-light bg-brand-high-pale text-brand-high-dark hover:shadow-sm'
                      }`}>
                    ...
                    <div className="p-2 grid grid-cols-3 gap-1">
                      {dept.majors.map(major => (
                        <button key={major.name} onClick={() => setSelectedMajor(major.name)}
                          className={`px-2 py-1.5 rounded-md text-[11px] font-medium text-left transition-all ${selectedMajor === major.name ? 'bg-brand-high text-white' : 'bg-gray-50 text-ink hover:bg-brand-high-pale hover:text-brand-high-dark'
                            }`}>
                          {major.name}
                        </button>
                      ))}
            ...
            <div className="grid grid-cols-2 gap-1.5 mb-3">
              {selMajorData.careers.map(career => (
                <button key={career} onClick={() => setSelectedCareer(career)}
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-left text-[12px] font-medium transition-all hover:-translate-y-px ${selectedCareer === career ? 'border-brand-high bg-brand-high text-white' : 'border-line bg-white text-ink hover:border-brand-high-light'
                    }`}>
        ...
        {step === 'keyword' && (
          <div>
            ...
            <div className="bg-gradient-to-br from-brand-high-pale to-blue-50 border border-brand-high-light rounded-xl p-3 mb-4">
              <div className="text-[10px] font-bold text-brand-high-dark uppercase tracking-wider mb-2">내 진로 계열 검사</div>
            ...
            <div className="flex items-center justify-between mb-1">
              <div className="text-[13px] font-bold text-ink">
                <span className="text-brand-high-dark">{selectedMajor}</span> 관심 키워드 (최대 5개)
              </div>
            ...
            {loadingKeywords ? (
              <div className="bg-white border border-line rounded-lg p-6 mb-4 text-center">
                <div className="inline-block w-5 h-5 border-2 border-gray-200 border-t-brand-high rounded-full animate-spin mb-2" />
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {keywordOptions.map(kw => (
                  <button key={kw} onClick={() => toggleKeyword(kw)}
                    className={`px-3 py-1.5 rounded-full text-[11px] font-bold border transition-all ${keywords.includes(kw) ? 'bg-brand-high text-white border-brand-high' : 'bg-white text-ink-secondary border-line hover:border-brand-high-light'
