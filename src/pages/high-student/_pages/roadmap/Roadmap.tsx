...
export default function Roadmap() {
  ...
  return (
    <div className="h-full overflow-y-auto px-8 py-7 box-border font-sans text-ink">
      ...
      >
        👁️ 로드맵 체크는 <b className="mx-1">원장님이</b> 해주세요. 🔗 바로가기 미션을 누르면 해당 화면으로 이동해요.
      </div>
          ...
          >
            ...
            {selected.missions.map((ms, mi) => {
              ...
              return (
                <div
                  key={mi}
                  onClick={() => ms.tab && navigate(`/high-student/${ms.tab}`)}
                  className={`rounded-lg px-3 py-2.5 mb-1.5 transition-all ${
                    ms.tab ? 'cursor-pointer hover:shadow-md hover:-translate-y-px' : ''
                  }`}
                  style={{
