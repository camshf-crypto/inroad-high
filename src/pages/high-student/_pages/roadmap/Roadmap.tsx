    ...
    <div style={{ height: '100%', overflowY: 'auto', padding: '28px 32px', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 30, height: 30, borderRadius: '50%', background: '#EEF2FF', color: '#3B5BDB', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600 }}>
            {student?.name?.[0]}
          </div>
          <div>
            <div style={{ fontSize: 15, fontWeight: 700 }}>{student?.name}</div>
            <div style={{ fontSize: 11, color: '#6B7280' }}>{grade} · {academy.academyName || '학원 미연결'}</div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 300px' : '1fr', gap: 16, alignItems: 'start' }}>
        ...
        {selected && selMonth !== null && (
          <div style={{ ...SEC, borderColor: '#3B5BDB', position: 'sticky', top: 0, maxHeight: 'calc(100vh - 160px)', overflowY: 'auto' }}>
