...
export default function TopicList() {
  ...
  const { data: researches = [], isLoading } = useMyResearches(gradeToNum(activeGrade))
  ...
  useEffect(() => { setSelResearchId(null) }, [activeGrade])

  useEffect(() => {
    if (!selResearchId && researches.length > 0) setSelResearchId(researches[0].id)
  ...
  const handleDeleteTopic = (topicId: string) => {
    if (window.confirm('탐구주제를 삭제할까요?')) {
      deleteResearch.mutate(topicId, { onSuccess: () => { if (selResearchId === topicId) setSelResearchId(null) } })
    }
  }

  const selectSetech = (s: any) => {
    setSelSetech(s)
    setNewTopic(p => ({ ...p, title: s.activity, subject: s.subject, content: s.topics.join('\n'), grade: p.grade }))
    setModalStep(2)
  }
  ...
  const addTopic = () => {
    ...
    createResearch.mutate(
      { topic: newTopic.title, subject: newTopic.subject || undefined, content: newTopic.content, grade: newTopic.grade },
      { onSuccess: (data: any) => { closeModal(); if (data?.id) setSelResearchId(data.id) } }
    )
  }
          ...
          <div className="flex-1 overflow-y-auto p-3">
            {isLoading ? (
              ...
              <div className="text-center py-10 text-ink-muted">
                ...
                <div className="text-[12px] mb-3">{activeGrade} 탐구주제가 없어요.</div>
                <div className="text-[11px] leading-relaxed">우측 상단 '+ 탐구주제 작성' 버튼으로<br />첫 탐구주제를 만들어보세요!</div>
              </div>
            ) : researches.map(topic => {
              const hasTeacher = topic.id === selResearchId && analyses.some(a => a.teacher_feedback)
              return (
                <div key={topic.id} onClick={() => setSelResearchId(topic.id)}
                  className={`relative border rounded-xl px-3 py-2.5 mb-1.5 cursor-pointer transition-all ${
                    selResearchId === topic.id ? 'border-brand-high bg-brand-high-pale shadow-[0_2px_8px_rgba(37,99,235,0.1)]' : 'border-line bg-white hover:border-brand-high-light hover:shadow-sm'
                  }`}>
                  <div className="flex gap-1 mb-1.5 flex-wrap">
                    {topic.status === 'completed' ? (
                      ...
                      <span className="text-[10px] font-bold text-brand-high-dark bg-brand-high-bg px-2 py-0.5 rounded-full">💬 선생님 답변 확인</span>
                    ) : (
                      <span className="text-[10px] font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-full">⏳ 피드백 대기</span>
                    )}
                  </div>
                  <div className="text-[12.5px] font-semibold text-ink mb-1.5 leading-tight line-clamp-2">{topic.topic}</div>
                  {topic.subject && (
                    <span className="text-[10px] font-bold text-brand-high-dark bg-brand-high-pale border border-brand-high-light px-2 py-0.5 rounded-full">{topic.subject}</span>
                  )}
                  <button onClick={e => { e.stopPropagation(); handleDeleteTopic(topic.id) }}
          ...
          {!selected ? (
            ...
            <>
              <div className="px-5 py-3 border-b border-line-light flex-shrink-0 bg-white">
                <div className="text-[15px] font-bold text-ink tracking-tight mb-1.5 truncate">{selected.topic}</div>
              ...
              <div className="px-4 py-3 border-t border-line-light bg-white flex-shrink-0">
                {isCompleted ? (
                  <div className="text-center py-3 text-[12px] text-ink-muted font-medium">✓ 완료 처리된 탐구주제예요. 선생님께 새 메시지 전송 불가.</div>
                ) : (
                  <>
                    ...
                    <div className="text-[10px] text-ink-muted mt-2 font-medium px-1">💡 선생님께 탐구 방향, 질문, 수정 내용을 자유롭게 작성해보세요</div>
                  </>
                )}
              </div>
            </>
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div className="px-6 py-4 border-b border-line-light flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-3">
                {modalStep === 2 && <button onClick={() => setModalStep(1)} className="text-ink-secondary hover:text-ink text-[14px] transition-colors">←</button>}
                <div>
                  <div className="text-[15px] font-bold text-ink tracking-tight">{modalStep === 1 ? '세특 Lite 참고하기' : '탐구주제 작성'}</div>
                  <div className="text-[11px] text-ink-muted mt-0.5 font-medium">{modalStep === 1 ? '학년, 학과, 과목을 선택해서 세특 사례를 찾아보세요' : '세특을 참고해서 내 탐구주제를 작성해요'}</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <div className="flex gap-1.5">
                  {[1, 2].map(s => <div key={s} className={`h-2 rounded-full transition-all ${s === modalStep ? 'w-5 bg-brand-high' : s < modalStep ? 'w-2 bg-brand-high-dark' : 'w-2 bg-line'}`} />)}
                </div>
                ...
                <div className="flex-1 overflow-hidden flex">
                  <div className="w-[260px] flex-shrink-0 border-r border-line-light flex flex-col bg-gray-50 overflow-hidden">
                    ...
                    <div className="px-4 py-3 border-t border-line-light flex-shrink-0">
                      <button onClick={() => setModalStep(2)} className="w-full h-9 bg-white text-ink-secondary border border-line rounded-lg text-[12px] font-medium hover:bg-gray-100 hover:border-ink-muted transition-all">
                        세특 없이 직접 작성
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 overflow-y-auto p-5">
                    {isFilterEmpty ? (
                      ...
                      <div key={s.id} className={`border rounded-xl mb-2.5 overflow-hidden transition-all ${expandedSetech === s.id ? 'border-brand-high bg-brand-high-pale/20' : 'border-line bg-white'}`}>
                        <div onClick={() => setExpandedSetech(expandedSetech === s.id ? null : s.id)} className="px-4 py-3 cursor-pointer flex items-center justify-between hover:bg-brand-high-pale/10 transition-colors">
                          ...
                          <div className="px-4 pb-4 border-t border-line-light">
                            {[{ label: '세특 활동', text: s.activity, id: `activity-${s.id}` }, { label: '탐구 역량', text: s.competency, id: `competency-${s.id}` }].map(item => (
                              <div key={item.id} className="mt-3">
                                <div className="flex items-center justify-between mb-1.5">
                                  ...
                                  <button onClick={() => copyText(item.text, item.id)} className={`text-[11px] font-semibold transition-colors ${copiedId === item.id ? 'text-emerald-600' : 'text-ink-secondary hover:text-brand-high-dark'}`}>
                            ...
                            <div className="mt-3 mb-3.5">
                              <div className="flex items-center justify-between mb-1.5">
                                ...
                                <button onClick={() => copyText(s.topics.join('\n'), `topics-${s.id}`)} className={`text-[11px] font-semibold transition-colors ${copiedId === `topics-${s.id}` ? 'text-emerald-600' : 'text-ink-secondary hover:text-brand-high-dark'}`}>
                ...
                <div className="flex flex-col gap-3.5">
                  ...
                  <div>
                    ...
                    <input value={newTopic.title} onChange={e => setNewTopic(p => ({ ...p, title: e.target.value }))}
                      placeholder={concept?.major ? `예: ${concept.career || concept.custom_goal}이 되기 위한 ~ 탐구` : '예: 기후변화가 농업 생산량에 미치는 영향 분석'}
                  ...
                  <div>
                    <label className="text-[11px] font-semibold text-ink-secondary mb-1 block">
                      ...
                      <span className="ml-1.5 text-[10px] font-bold text-brand-high-dark bg-brand-high-pale border border-brand-high-light px-1.5 py-0.5 rounded-full">고{newTopic.grade} 과목</span>
                    </label>
                    <SubjectSelect value={newTopic.subject} onChange={v => setNewTopic(p => ({ ...p, subject: v }))} grade={newTopic.grade} placeholder="과목을 선택하거나 직접 입력해주세요" multi />
                  </div>
                  <div>
                    ...
                    <textarea value={newTopic.content} onChange={e => setNewTopic(p => ({ ...p, content: e.target.value }))}
                      placeholder={concept?.major ? `${concept.major} 진학 / ${concept.career || concept.custom_goal} 방향에 맞춰 어떤 내용을 탐구하고 싶은지 자세히 작성해주세요` : '어떤 내용을 탐구하고 싶은지 자세히 작성해주세요'}
                      rows={6}
