...
export default function RoadmapBoard({
  ...
  /** 라인별로 실제 존재하는 칸만 추린다 (빈 학년은 아예 없음) */
  const cellsByLine = useMemo(() => {
    const m = new Map<string, Cell[]>()
    for (const l of lines) {
      const cells: Cell[] = []
      for (const g of GRADES) {
        const node = pickNode(board, l.id, g)
        if (!node) continue
        cells.push({
          grade: g,
          node,
          candidates: board.nodesByLine.get(l.id)?.get(g)?.length ?? 1,
  ...
  const drawn = lines.filter((l) => cellsByLine.has(l.id))
            ...
            return (
              ...
              >
                ...
                {cells.slice(0, -1).map((cell, i) => {
                  ...
                  const x1 = COL_X[cell.grade - 1] + NODE_W / 2 - 6
                  const x2 = COL_X[next.grade - 1] - NODE_W / 2 + 6
                  const mid = (x1 + x2) / 2
