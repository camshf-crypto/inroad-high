...
export default function Past() {
  ...
  const printAnswers = () => {
    const answeredQs = curQuestions.filter(q => q.answered)
...
.badge-q { background: #EEF2FF; color: #3B5BDB; }
.badge-type-공통 { background: #EEF2FF; color: #3B5BDB; border: 0.5px solid #BAC8FF; }
.badge-type-전공 { background: #F0FDF4; color: #059669; border: 0.5px solid #6EE7B7; }
.badge-type-인성 { background: #FFF3E8; color: #D97706; border: 0.5px solid #FDBA74; }
...
.answer-box { background: #F8F9FA; border-radius: 4px; padding: 5px 8px; font-size: 11px; line-height: 1.5; }
.final-box { background: #ECFDF5; border: 0.5px solid #6EE7B7; border-radius: 4px; padding: 5px 8px; font-size: 11px; line-height: 1.5; }
.tail-box { background: #EEF2FF; border-radius: 4px; padding: 4px 8px; margin-top: 4px; }
.tail-q { font-size: 11px; color: #3B5BDB; font-weight: 600; margin-bottom: 2px; }
  ...
  ${answeredQs.map((q, i) => `
    <div class="block">
      ...
      ${q.tails && q.tails.length > 0 ? `
        ...
        ${q.tails.map((t: string) => `
          <div class="tail-box">
            <div class="tail-q">↳ ${t}</div>
            <div style="font-size:12px; color:#6B7280; margin-top:4px;">답변 준비 중</div>
          </div>
        `).join('')}
      ` : ''}
    </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, height: 'calc(100vh - 50px)', overflow: 'hidden', padding: '20px 24px' }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexShrink: 0, alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={printAnswers}
          style={{ padding: '7px 16px', background: '#fff', color: '#3B5BDB', border: '0.5px solid #3B5BDB', borderRadius: 99, fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit', flexShrink: 0, marginLeft: 'auto' }}>
          🖨️ 최종 답변집 인쇄
        </button>

      </div>

      {/* 오른쪽 끝 인쇄 버튼 - 위 div 닫히고 나서 */}
