...
export default function Expect() {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return
    if (!selDept[grade]) { alert('전공학과를 먼저 선택해주세요!'); return }
            ...
            {[{ key: 'questions', label: '예상질문' }, { key: 'upload', label: '생기부 업로드' }].map(t => (
              <button key={t.key} onClick={() => setActiveTab(t.key as any)}
                className={`flex-1 py-3 text-[13px] font-semibold transition-colors border-b-2 ${
                  activeTab === t.key ? 'text-brand-high-dark border-brand-high' : 'text-ink-secondary border-transparent hover:text-ink'
                }`}>
                {t.label}
              </button>
            ))}
            ...
            <>
              <div className="px-4 py-2.5 border-b border-line-light flex-shrink-0">
                <div className="text-[12px] text-ink-secondary font-medium leading-[1.7]">
                  총 <span className="text-brand-high-dark font-bold">{questions.length}개</span><br />
                ...
                {loadingQ ? (
                  ...
                  return (
                    <div key={q.id} onClick={() => setSelQId(q.id)}
                      ...
                      <span className="text-[10px] font-bold text-brand-high-dark bg-brand-high-pale px-2 py-0.5 rounded-full inline-block mb-1.5">질문 {i + 1}</span>
          ...
          {activeTab === 'upload' && (
            <div className="flex-1 overflow-y-auto p-4">
              ...
              <div className="mb-4">
                ...
                <div className="text-[12px] text-ink-secondary mb-2 leading-relaxed">지원 희망 학과를 선택하면 더 정확한 예상질문을 만들어 드려요.</div>
                <div className="relative">
                  <div onClick={() => setDeptDropOpen(prev => ({ ...prev, [grade]: true }))}
                    className={`flex items-center gap-2 border rounded-lg px-3 py-2.5 bg-white cursor-text transition-colors ${deptDropOpen[grade] ? 'border-brand-high' : 'border-line'}`}>
                        ...
                        {filteredDepts.length === 0
                          ...
                          : filteredDepts.map((d, i) => (
                            <div key={i}
                              ...
                              className={`px-3 py-2 text-[12px] cursor-pointer transition-colors ${selDept[grade] === d ? 'bg-brand-high-pale text-brand-high-dark font-semibold' : 'text-ink hover:bg-brand-high-pale/50'} ${i < filteredDepts.length - 1 ? 'border-b border-line-light' : ''}`}>
                              {d}
                            </div>
                          ))}
                      </div>
                ...
                {uploadedPdf ? (
                  <>
                    ...
                    <div className="flex gap-2">
                      <label htmlFor={`upload-${grade}`} className="flex-1 py-2 bg-white text-brand-high-dark border border-brand-high rounded-lg text-[12px] font-bold text-center cursor-pointer hover:bg-brand-high-pale transition-all">🔄 다시 업로드</label>
                  ...
                  <label htmlFor={selDept[grade] ? `upload-${grade}` : undefined}
                    ...
                    className={`block border-[1.5px] border-dashed rounded-xl py-7 text-center transition-all ${selDept[grade] ? 'border-brand-high-light bg-brand-high-pale/30 cursor-pointer hover:bg-brand-high-pale' : 'border-line bg-gray-50 cursor-not-allowed'}`}>
                    <div className="text-3xl mb-2">📄</div>
                    <div className={`text-[13px] font-bold ${selDept[grade] ? 'text-brand-high-dark' : 'text-ink-muted'}`}>PDF 파일 업로드</div>
                    <div className="text-[11px] text-ink-muted mt-1 font-medium">{selDept[grade] ? `${grade} 생기부를 업로드해주세요` : '전공학과를 먼저 선택해주세요'}</div>
                  </label>
                )}
              </div>
            </div>
                      ...
                      <div key={i} className="flex-1 flex flex-col items-center gap-1 relative">
                        ...
                        <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold z-10 border-2 transition-all ${
                          isDone ? 'bg-emerald-500 text-white border-emerald-500'
                            : isOn ? 'bg-brand-high text-white border-brand-high shadow-[0_2px_8px_rgba(37,99,235,0.3)]'
                      ...
                      <div key={fu.id} className="mb-3">
                        ...
                        {fu.student_answer ? (
                          ...
                          <Step5TailBox
                            ...
                            questionId={selQ.id}
                            tailIndex={i}
                            onSubmit={(text, clearDraft) => handleSubmitTail(fu.id, text, clearDraft)}
