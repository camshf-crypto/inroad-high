      ...
      >
        ...
        <nav className="flex-1 px-1.5 py-2 overflow-hidden">
          {menus.map(m => {
            ...
            return (
              <div key={m.path} onClick={() => navigate(m.path)} title={collapsed ? m.label : ''}
                className="flex items-center rounded-lg mb-0.5 cursor-pointer transition-all"
                style={{
                  background: isActive ? accentBg : 'transparent',
                  color: isActive ? accentDark : '#6B7280',
                  fontWeight: isActive ? 700 : 500,
                  boxShadow: isActive ? `0 2px 6px ${accentShadow}` : 'none',
                  padding: collapsed ? '8px 0' : '8px 10px',
                  justifyContent: collapsed ? 'center' : 'flex-start',
                  gap: collapsed ? 0 : '8px',
                }}
                onMouseEnter={e => { if (!isActive) { e.currentTarget.style.background = accentBg; e.currentTarget.style.color = accentDark } }}
              ...
              >
                ...
                {!collapsed && isActive && <div className="ml-auto w-1 h-3.5 rounded-full" style={{ background: accent }} />}
              </div>
            )
          })}
        </nav>
