...
export default function TopicTab({ student, onOpenChat }: { student: any, onOpenChat: (type: 'topic' | 'book', context: string) => void }) {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto' }}>
      <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
        {['전체', '고1', '고2', '고3'].map(g => (
          <div key={g} onClick={() => setTopicGrade(g)} style={{ padding: '5px 14px', borderRadius: 99, fontSize: 12, cursor: 'pointer', background: topicGrade === g ? '#1a1a1a' : '#fff', color: topicGrade === g ? '#fff' : '#6B7280', border: `0.5px solid ${topicGrade === g ? '#1a1a1a' : '#E5E7EB'}` }}>{g}</div>
        ))}
      </div>
      {TOPICS.filter(t => topicGrade === '전체' || t.grade === topicGrade).map(topic => {
        ...
        return (
          <div key={topic.id} style={SEC}>
            <div onClick={() => setOpenTopics(prev => ({ ...prev, [topic.id]: !isOpen }))} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
              <div style={{ flex: 1 }}>
            ...
            {isOpen && (
              <div style={{ marginTop: 16, borderTop: '0.5px solid #E5E7EB', paddingTop: 16 }}>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <textarea value={feedback[topic.id] || ''} onChange={e => setFeedback(prev => ({ ...prev, [topic.id]: e.target.value }))} placeholder="탐구주제에 대한 피드백을 작성해주세요..." rows={3} style={{ width: '100%', border: '0.5px solid #E5E7EB', borderRadius: 8, padding: '10px 12px', fontSize: 13, outline: 'none', resize: 'vertical', fontFamily: 'inherit', lineHeight: 1.6 }} />
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button style={{ padding: '8px 16px', background: '#3B5BDB', color: '#fff', border: 'none', borderRadius: 7, fontSize: 12, cursor: 'pointer' }}>피드백 전달</button>
                  <button onClick={() => onOpenChat('topic', topic.title)} style={{ padding: '8px 16px', background: '#fff', color: '#3B5BDB', border: '0.5px solid #3B5BDB', borderRadius: 7, fontSize: 12, cursor: 'pointer' }}>✨ 챗봇으로 고도화</button>
                  <button onClick={() => setTopicStatus(prev => ({ ...prev, [topic.id]: '완료' }))} style={{ padding: '8px 16px', background: curStatus === '완료' ? '#059669' : '#E5E7EB', color: curStatus === '완료' ? '#fff' : '#9CA3AF', border: 'none', borderRadius: 7, fontSize: 12, cursor: 'pointer', marginLeft: 'auto' }}>완료 처리</button>
                </div>
              </div>
            )}
          </div>
      ...
      {TOPICS.filter(t => topicGrade === '전체' || t.grade === topicGrade).length === 0 && (
        <div style={{ textAlign: 'center', color: '#6B7280', fontSize: 13, padding: '40px 0' }}>해당 학년의 탐구주제가 없어요.</div>
      )}
    </div>
  )
}