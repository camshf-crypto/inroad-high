...
// AI 분석 mock (음성 분석은 나중에 Claude API + Whisper)
const AI_ANALYSIS_MOCK = {
  speed: {
    mine: 135,
    avg: 170,
    comment:
      "말의 속도가 평균보다 약간 느려요. 자신감 있게 말하는 연습을 해보세요.",
  },
  habits: ["음...", "그래서", "어..."],
  pause: {
    level: "양호",
...
export default function MiddleSimulationTab({ student }: { student: any }) {
  ...
  // ⭐ DB 훅
  const { data: simulations = [], isLoading } =
    useStudentSimulations(studentId);
  ...
  // AI 분석 표시 (mock 데이터 — 시뮬레이션이 있고 답변완료된 경우)
  const showAiAnalysis = selSim?.audio_url;
      ...
      <div className="w-[340px] flex-shrink-0 bg-white border border-line rounded-2xl flex flex-col overflow-hidden shadow-[0_2px_8px_rgba(15,23,42,0.04)]">
        ...
        <div className="px-3 py-2.5 border-b border-line flex-shrink-0">
          <div className="flex gap-1 flex-wrap">
            {["all", "기출문제", "자소서 예상질문", "생기부 예상질문"].map(
              (t) => {
          ...
          <>
            ...
            <div className="px-5 py-4 border-b border-line flex-shrink-0">
              ...
              <div className="text-[11px] font-semibold text-ink-secondary mt-0.5">
                ...
                {selSim.duration || "-"} · 📋 {(selSim.questions || []).length}
                문항
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3">
              ...
              <div className="bg-white border border-line rounded-xl px-5 py-4">
                ...
                {!selSim.transcript && (
                  <div className="mt-3 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-[11px] text-amber-700 text-center">
                    💡 음성→텍스트 변환은 곧 추가됩니다 (AI 통합 후)
                  </div>
                )}
              </div>
      ...
      {deleteTarget !== null && (
        ...
        >
          ...
          >
            ...
            <div className="text-[17px] font-extrabold text-ink mb-2">
              시뮬레이션을 삭제하시겠어요?
            </div>
