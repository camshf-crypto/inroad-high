...
// 메인 컴포넌트
// ─────────────────────────────────────────────
export default function Record({ student }: {
  student: any
}) {
          ...
          <table className="w-full border-collapse border border-gray-700">
            <thead>
              <tr className="bg-gray-100">
                ...
                {!inModal && (
                  <th className="border border-gray-700 font-bold text-ink text-center w-28 px-2 py-1.5 text-[10px]">
                    학기 정리
                  </th>
                )}
              </tr>
            ...
            <tbody>
              {setechSubjects.length === 0 ? (
                ...
                setechSubjects.map((subject: string) => {
                  ...
                  const finKey = `finalize-${subject}`
                  const isFinalizing = generating === finKey
                      ...
                      <td className={`border border-gray-700 font-semibold text-ink text-center bg-gray-50 align-top whitespace-nowrap ${inModal ? 'px-3 py-2 text-[12px]' : 'px-2 py-1.5 text-[10px]'}`}>
                        <div className="flex flex-col items-center gap-1">
                          ...
                          {!inModal && item?.generation_mode === 'finalized' && (
                            <span className="text-[8px] font-bold text-purple-700 bg-purple-50 border border-purple-200 px-1.5 py-0.5 rounded-full whitespace-nowrap">
                              압축됨
                            </span>
                          )}
                        </div>
                      ...
                      {!inModal && (
                        <td className="border border-gray-700 align-top p-1.5 text-center">
                          <button
                            ...
                            style={{
                              background: isFinalizing ? '#E5E7EB' : THEME.accent,
                              color: isFinalizing ? '#9CA3AF' : '#fff',
                            }}
                          >
                            {isFinalizing ? '⏳' : '📝 학기 정리'}
                          </button>
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3">
              ...
              <div>
                ...
                <div className="bg-blue-50/40 border border-blue-100 rounded-lg px-3 py-2 text-[11.5px] text-ink leading-[1.7] whitespace-pre-wrap">
                  {selectedSubmission.resubmitted_text || selectedSubmission.answer_text || '(답변 없음)'}
                </div>
              </div>
              ...
              <div className="border-t border-line pt-3">
                <button
                  ...
                  style={{
                    background: locked ? '#9CA3AF' : THEME.accent,
                    boxShadow: locked ? 'none' : `0 2px 8px ${THEME.accentShadow}`,
                  }}
                >
                  {locked
                    ? '🔒 학기 마감됨'
                    : generating === `accumulate-${selectedSubmission.id}`
                      ? '⏳ AI 분석 중... (10~20초)'
                      : '✨ 세특에 AI 누적하기'}
                </button>
        ...
        <div className="px-4 py-3 border-b border-line flex-shrink-0 flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="flex gap-2 items-center">
            <button
              ...
              style={{
                background: lockChip.bg,
                color: lockChip.color,
                borderColor: lockChip.border,
              }}
            >
              {lockChip.label}
            </button>
...
function LockModal({ academyId, grade, semester, gradeLabel, currentLock, onClose }: {
  ...
  const [deadlineDate, setDeadlineDate] = useState(
    currentLock?.deadline_at
      ? new Date(currentLock.deadline_at).toISOString().slice(0, 10)
      : ''
  )
  const [deadlineTime, setDeadlineTime] = useState(
    currentLock?.deadline_at
      ? new Date(currentLock.deadline_at).toISOString().slice(11, 16)
      : '23:59'
  )
  ...
  const handleSave = () => {
    ...
    if (deadlineDate) {
      deadlineISO = new Date(`${deadlineDate}T${deadlineTime}:00`).toISOString()
    }
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl flex flex-col overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.3)] w-[480px] max-h-[90vh]"
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          <div className="mb-5">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 border border-line rounded-lg text-[13px] focus:outline-none focus:border-brand-primary"
              />
              <input
                type="time"
                value={deadlineTime}
                onChange={e => setDeadlineTime(e.target.value)}
                className="w-28 px-3 py-2 border border-line rounded-lg text-[13px] focus:outline-none focus:border-brand-primary"
              />
            </div>
            <div className="text-[10px] text-ink-muted mt-1">
              이 시각이 지나면 자동으로 마감 처리돼요.
            </div>
          </div>
          ...
          <div className="mb-5">
            ...
            <textarea
              ...
              placeholder="예: 기말고사 끝나기 전까지 다 제출하세요."
              rows={3}
              className="w-full px-3 py-2 border border-line rounded-lg text-[12px] resize-none focus:outline-none focus:border-brand-primary"
            />
          </div>
        ...
        <div className="px-6 py-4 border-t border-line flex-shrink-0 flex justify-between items-center">
          {currentLock ? (
            <button
              ...
              className="px-3 py-2 text-[12px] font-bold text-red-600 border border-red-200 rounded-lg hover:bg-red-50 transition-colors disabled:opacity-60"
            >
              마감 해제
            </button>
          ) : <div />}
...
function FinalizeModal({ subject, submissions, existingContent, gradeLabel, semester, isGenerating, onClose, onSubmit }: {
  subject: string
  submissions: SuhaengSubmission[]
  existingContent: string | null
  gradeLabel: string
  semester: number
  isGenerating: boolean
  onClose: () => void
  onSubmit: (emphasis: string, ids: string[]) => void
}) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-2xl flex flex-col overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.3)] w-[560px] max-h-[90vh]"
        onClick={e => e.stopPropagation()}
      >
        <div className="px-6 py-4 border-b border-line flex-shrink-0 flex items-center justify-between">
          <div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {existingContent && (
            <div className="mb-5">
              ...
              <div className="bg-gray-50 border border-line rounded-lg p-3 text-[11px] text-ink-secondary leading-[1.7] whitespace-pre-wrap max-h-32 overflow-y-auto">
                {existingContent}
              </div>
            </div>
          )}
          ...
          <div className="mb-5">
            <div className="text-[12px] font-bold text-ink mb-2">
              ☑️ 반영할 수행평가 선택 ({selectedIds.length}/{submissions.length})
            </div>
            {submissions.length === 0 ? (
              <div className="text-[11px] text-ink-muted text-center py-4 bg-gray-50 rounded-lg">
                이 학기 {subject} 수행평가가 없어요.
              </div>
            ) : (
              <div className="flex flex-col gap-1.5 max-h-48 overflow-y-auto">
                {submissions.map((sub: SuhaengSubmission) => {
                  ...
                  return (
                    <label
                      key={sub.id}
                      className="flex items-start gap-2 px-3 py-2 border rounded-lg cursor-pointer transition-all"
                      style={{
                        background: isSelected ? THEME.accentBg : '#fff',
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-[10px] text-ink-muted">
                          {sub.question_category} · {sub.submitted_at ? new Date(sub.submitted_at).toLocaleDateString('ko-KR') : '—'}
                        </div>
                      </div>
                    </label>
                  )
              ...
              {EMPHASIS_OPTIONS.map(opt => {
                ...
                return (
                  <button
                    ...
                    className="px-3 py-2 rounded-lg text-[11.5px] font-bold transition-all border text-left"
                    style={{
        ...
        <div className="px-6 py-4 border-t border-line flex-shrink-0 flex justify-end gap-2">
          <button
            ...
            className="px-4 py-2 bg-white text-ink-secondary border border-line rounded-lg text-[12px] font-semibold hover:bg-gray-50 transition-colors disabled:opacity-60"
          >
            취소
          </button>
          <button
            onClick={() => onSubmit(emphasis, selectedIds)}
