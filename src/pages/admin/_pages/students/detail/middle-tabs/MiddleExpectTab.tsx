// src/pages/admin/_pages/students/detail/middle-tabs/MiddleExpectTab.tsx
// 🌱 중등 생기부 예상질문 어드민 탭 (PDF 미리보기 없는 버전)
...
} from '@/pages/admin/_hooks/middle/useMiddleSaenggibuQuestions'
...
const TAG_OPTIONS = [
  ...
  '수상경력',
  '자격증',
  '1학년 자율활동', '2학년 자율활동', '3학년 자율활동',
  '1학년 동아리활동', '2학년 동아리활동', '3학년 동아리활동',
  '1학년 봉사활동', '2학년 봉사활동', '3학년 봉사활동',
  '1학년 진로활동', '2학년 진로활동', '3학년 진로활동',
  '1학년 자유학기활동',
  '2학년 자유학기활동',
  '1학년 교과성적', '2학년 교과성적', '3학년 교과성적',
  '1학년 세특', '2학년 세특', '3학년 세특',
  '1학년 독서활동', '2학년 독서활동', '3학년 독서활동',
    ...
    <>
      ...
      {analysis.authenticity_concerns && analysis.authenticity_concerns.length > 0 && (
        <div style={{ marginBottom: 12 }}>
          ...
          {analysis.authenticity_concerns.map((a: string, i: number) => (
...
export default function MiddleExpectTab({ student }: { student: any }) {
  ...
  const [selGrade, setSelGrade] = useState('중1')
  ...
  const selQ: ExpectQuestion | null = questions.find((q: ExpectQuestion) => q.id === selQId) ?? null
  const step = selQ ? getQuestionStep(selQ, analyses) : 0
  const round1 = analyses.find((a: ExpectAnalysis) => a.round === 1)
  const round2 = analyses.find((a: ExpectAnalysis) => a.round === 2)

  const aiQuestionCount = questions.filter((q: ExpectQuestion) => q.source_type === 'ai').length
  ...
  const handleAddManual = async () => {
    const validItems = manualItems.filter((item: ManualQuestion) => item.question.trim())
  ...
  const removeManualRow = (index: number) => setManualItems(prev => prev.filter((_: ManualQuestion, i: number) => i !== index))
  const updateManualRow = (index: number, updates: Partial<ManualQuestion>) =>
    setManualItems(prev => prev.map((item: ManualQuestion, i: number) => i === index ? { ...item, ...updates } : item))
  ...
  const openAiTailModal = () => {
    ...
    genAITails.mutate({
      ...
      onSuccess: (data: string[]) => setAiTails(data),
      onError: (err: any) => {
        alert('AI 꼬리질문 생성 실패:\n' + (err?.message || '알 수 없는 오류'))
  ...
  const deliverAiTails = () => {
    ...
    const selected = selectedAiTails.map((i: number) => aiTails[i])
    saveAITails.mutate({
      question_id: selQ.id, student_id: studentId,
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {questions.filter((q: ExpectQuestion) => q.question_status === 'draft').length > 0 && (
            <button onClick={handlePublishAll} disabled={publishAll.isPending}
              style={{
                padding: '6px 14px', background: '#047857', color: '#fff',
                border: 'none', borderRadius: 99, fontSize: 12, fontWeight: 700, cursor: 'pointer',
              }}>
              {publishAll.isPending ? '처리중...' : `👁 ${questions.filter((q: ExpectQuestion) => q.question_status === 'draft').length}개 학생에게 전체 게시`}
            </button>
          )}
        </div>
      </div>
                    ...
                    <>
                      ...
                      <button onClick={handleGenerateAI} disabled={generateAI.isPending}
                        style={{
                          ...
                          background: generateAI.isPending ? '#A7F3D0' : THEME.accent,
          ...
          {leftTab === 'answers' && (
            <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
              <div style={{ fontSize: 12, color: '#6B7280', marginBottom: 10, padding: '0 4px', lineHeight: 1.7 }}>
                ...
                게시 <span style={{ color: '#059669', fontWeight: 700 }}>{questions.filter((q: ExpectQuestion) => q.question_status === 'published').length}개</span>
                <br />
                답변완료 <span style={{ color: '#059669', fontWeight: 700 }}>{questions.filter((q: ExpectQuestion) => !!q.student_answer).length}개</span> ·
                미답변 <span style={{ color: '#D97706', fontWeight: 700 }}>{questions.filter((q: ExpectQuestion) => !q.student_answer).length}개</span>
              </div>
              ...
              ) : questions.map((q: ExpectQuestion, i: number) => {
            ...
            <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, gap: 8, flexWrap: 'wrap' }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 600, color: '#1a1a1a' }}>
                      질문 {questions.findIndex((q: ExpectQuestion) => q.id === selQ.id) + 1}
                    </div>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div>
                  ...
                  ) : followups.map((fu: ExpectFollowup, i: number) => (
          ...
          <div style={{ padding: '14px 18px', borderBottom: '1px solid #E5E7EB', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: THEME.accentBg }}>
            <div>
              ...
              <div style={{ fontSize: 11, color: '#6B7280', marginTop: 2 }}>
                {student.name} · 질문 {questions.findIndex((q: ExpectQuestion) => q.id === selQ.id) + 1}
              </div>
            </div>
            <button
          ...
          {currentRoundHasAnalysis && !currentRoundFeedbackDone && (
            <div style={{
              ...
              <button
                ...
                style={{
                  width: '100%', padding: '12px 0',
                  background: aiSuggestLoading !== null ? '#A7F3D0' : THEME.accent,
                  color: '#fff',
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            {genAITails.isPending ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
                {aiTails.map((t: string, i: number) => (
                  <div key={i} onClick={() => setSelectedAiTails(prev => prev.includes(i) ? prev.filter((x: number) => x !== i) : [...prev, i])}
        ...
        <div onClick={() => setShowManualModal(false)}
          ...
          <div onClick={e => e.stopPropagation()}
            ...
            <div style={{ padding: '20px 28px 14px', borderBottom: '1px solid #E5E7EB', flexShrink: 0 }}>
              ...
              <div style={{ fontSize: 12, color: '#6B7280' }}>중학교 생기부 항목별로 질문을 만들어주세요 (총 29개 항목)</div>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px 28px' }}>
              {manualItems.map((item: ManualQuestion, idx: number) => (
                <div key={idx} style={{
                  ...
                  <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    <div style={{ width: 170, flexShrink: 0 }}>
                      <select value={item.tag} onChange={e => updateManualRow(idx, { tag: e.target.value })}
                        ...
                        {TAG_OPTIONS.map((t: string) => <option key={t} value={t}>{t}</option>)}
                      </select>
                    </div>
                    <div style={{ flex: 1 }}>
              ...
              <button onClick={addManualRow}
                style={{
                  ...
                  border: `1.5px dashed ${THEME.accentBorder}`, borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer',
                }}>+ 질문 추가</button>
            </div>
            ...
            <div style={{ padding: '14px 28px 20px', borderTop: '1px solid #E5E7EB', display: 'flex', gap: 8, flexShrink: 0 }}>
              ...
              <button onClick={handleAddManual}
                disabled={manualItems.filter((i: ManualQuestion) => i.question.trim()).length === 0 || addManual.isPending}
                style={{
                  flex: 2, height: 42,
                  background: manualItems.filter((i: ManualQuestion) => i.question.trim()).length > 0 && !addManual.isPending ? THEME.accent : '#E5E7EB',
                  color: manualItems.filter((i: ManualQuestion) => i.question.trim()).length > 0 && !addManual.isPending ? '#fff' : '#9CA3AF',
                  border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700,
                  cursor: manualItems.filter((i: ManualQuestion) => i.question.trim()).length > 0 && !addManual.isPending ? 'pointer' : 'not-allowed',
                }}>
                {addManual.isPending ? '저장중...' : `${manualItems.filter((i: ManualQuestion) => i.question.trim()).length}개 질문 저장`}
              </button>
            </div>
          </div>
        </div>
