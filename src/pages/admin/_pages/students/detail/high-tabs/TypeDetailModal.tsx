// src/pages/admin/_pages/students/detail/high-tabs/TypeDetailModal.tsx

import { useEffect } from 'react'
import { TYPE_DETAIL } from './TypeDetailData'
import { TYPE_NAMES } from '@/pages/high-student/_pages/concept/questions'
...
export default function TypeDetailModal({ typeCode, open, onClose }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        {/* 헤더 (그라데이션 배경) */}
        <div className="relative bg-gradient-to-br from-brand-high-pale to-blue-50 border-b border-brand-high-light px-8 py-6 flex-shrink-0">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[11px] font-bold text-brand-high-dark uppercase tracking-wider">진단 유형 해설</span>
                <span className="text-[10px] font-bold text-white bg-brand-high px-2 py-0.5 rounded-full">
                  {typeCode}
                </span>
              </div>
              <div className="text-[22px] font-extrabold text-brand-high-dark mb-1">{typeName}</div>
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6">
          ...
          <Section title="추천 학과 TOP 6" emoji="🏫">
            <div className="grid grid-cols-2 gap-2.5">
              {detail.majors.map((m, idx) => (
                <div
                  key={m.name}
                  className="bg-white border border-line rounded-xl p-3.5 hover:border-brand-high-light hover:bg-brand-high-pale/30 transition-colors"
                >
                  <div className="flex items-start gap-2.5">
                    <div className="w-6 h-6 rounded-full bg-brand-high-pale text-brand-high-dark text-[11px] font-bold flex items-center justify-center flex-shrink-0">
                      {idx + 1}
                    </div>
          ...
          <Section title="대학생활 적응 전략" emoji="🎓">
            <div className="bg-brand-high-pale/40 border border-brand-high-light rounded-xl p-4">
          ...
          <Section title="학부모님께 드리는 말씀" emoji="💬" last>
            <div className="bg-gradient-to-br from-brand-high-pale to-blue-50 border border-brand-high-light rounded-xl p-5">
        ...
        <div className="flex-shrink-0 border-t border-line bg-gray-50 px-8 py-4 flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="px-5 py-2 bg-brand-high text-white rounded-lg text-[13px] font-bold hover:bg-brand-high-dark transition-colors"
          >
            닫기
          </button>
        </div>
