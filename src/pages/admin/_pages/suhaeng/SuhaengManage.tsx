...
import {
  useHighSuhaengQuestions,
  useCreateHighSuhaengQuestion,
  useUpdateHighSuhaengQuestion,
  useDeleteHighSuhaengQuestion,
  useHighAcademyStudents,
  useHighQuestionAssignments,
  useSyncHighQuestionAssignments,
  type HighSuhaengQuestion,
  type HighSuhaengType,
  type HighSuhaengGrade,
...
function MiddleQuestionFormModal({ initial, onClose, onSave, saving }: {
  ...
  const [evalCriteria, setEvalCriteria] = useState<EvalItem[]>(
    (initial as any)?.eval_criteria?.length > 0 ? (initial as any).eval_criteria : MIDDLE_DEFAULT_EVAL['논술형']
  )
  ...
  const handleTypeChange = (t: SuhaengType) => {
    setType(t)
    if (!isEdit) setEvalCriteria(MIDDLE_DEFAULT_EVAL[t])
  }
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('middle-suhaeng-question', {
        body: { level: 'middle', subject, type, grade: grade === '전체' ? '중2' : grade },
      })
  ...
  const handleSave = () => {
    ...
    onSave({ level: 'middle', type, grade, subject, title, content, min_chars: info.hasChars ? Number(minChars) : null, max_chars: info.hasChars ? Number(maxChars) : null, eval_criteria: evalCriteria, is_draft: false })
  }
    ...
    <div className="fixed inset-0 z-[400] flex items-center justify-center"
      ...
      <div className="bg-white rounded-2xl flex flex-col overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.3)] w-[660px] max-h-[94vh]"
        ...
        <div className="px-6 py-4 border-b border-line flex-shrink-0 flex items-center justify-between">
          <div>
            <div className="text-[16px] font-extrabold text-ink">{isEdit ? '문제 수정' : '새 문제 만들기'}</div>
            <div className="text-[12px] text-ink-secondary mt-0.5">중등 수행평가</div>
          </div>
        ...
        {!isEdit && (
          <div className="flex border-b border-line flex-shrink-0">
            {[{ key: 'direct', label: '직접 입력' }, { key: 'ai', label: 'AI가 만들어주기' }].map(m => (
              <button key={m.key} onClick={() => setMode(m.key as 'direct' | 'ai')}
          ...
          {mode === 'ai' && !isEdit && (
            <div className="bg-gradient-to-br from-emerald-50 to-blue-50 border border-emerald-200 rounded-2xl p-5">
              <div className="mb-4">
                ...
                <div className="text-[11px] text-ink-secondary mt-0.5">학년·과목·유형 선택 후 AI가 문제를 만들어드려요</div>
              </div>
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {MIDDLE_SUBJECTS.map(s => (
                    <button key={s} onClick={() => setSubject(s)}
                      ...
                      style={{ background: subject === s ? MIDDLE_THEME.accent : '#fff', color: subject === s ? '#fff' : '#6B7280', borderColor: subject === s ? MIDDLE_THEME.accent : '#E5E7EB' }}>
                      {s}
                    </button>
                  ))}
                </div>
              ...
              <div className="mb-3">
                ...
                <div className="grid grid-cols-5 gap-1.5">
                  {MIDDLE_TYPE_LIST.map(t => (
                    <button key={t} onClick={() => handleTypeChange(t)}
                      className="py-2 rounded-xl text-center transition-all border"
                      style={{ background: type === t ? MIDDLE_THEME.accent : '#fff', color: type === t ? '#fff' : '#374151', borderColor: type === t ? MIDDLE_THEME.accent : '#E5E7EB' }}>
              ...
              <button onClick={handleAiGenerate} disabled={aiLoading || !subject}
                ...
                style={{ background: MIDDLE_THEME.accent, boxShadow: `0 4px 12px ${MIDDLE_THEME.accentShadow}` }}>
                ...
                <div className="grid grid-cols-5 gap-2">
                  {MIDDLE_TYPE_LIST.map(t => (
                    <button key={t} onClick={() => handleTypeChange(t)}
                      ...
                      style={{ background: type === t ? MIDDLE_THEME.accent : '#fff', color: type === t ? '#fff' : '#374151', borderColor: type === t ? MIDDLE_THEME.accent : '#E5E7EB', boxShadow: type === t ? `0 4px 12px ${MIDDLE_THEME.accentShadow}` : 'none' }}>
                      <span className="text-[11px] font-bold">{t}</span>
                    </button>
                  ))}
                </div>
        ...
        <div className="px-6 py-4 border-t border-line flex-shrink-0 flex justify-end gap-2">
          ...
          {(mode === 'direct' || isEdit || title) && (
            <>
              <button onClick={() => onSave({ level: 'middle', type, grade, subject, title, content, min_chars: info.hasChars ? Number(minChars) : null, max_chars: info.hasChars ? Number(maxChars) : null, eval_criteria: evalCriteria, is_draft: true })}
              ...
              <button onClick={handleSave} disabled={saving}
                ...
                style={{ background: MIDDLE_THEME.accent, boxShadow: `0 2px 8px ${MIDDLE_THEME.accentShadow}` }}>
...
function MiddleAssignModal({ question, onClose }: { question: SuhaengQuestion; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!loadingAssignments) setSelectedIds(assignedIds)
  }, [loadingAssignments, assignments.length])

  const grades = ['전체', '중1', '중2', '중3']
  const filteredStudents = (allStudents as AcademyStudent[]).filter(s => {
  ...
  const toggleStudent = (id: string) =>
    setSelectedIds(prev => prev.includes(id) ? prev.filter((x: string) => x !== id) : [...prev, id])
  const toggleAll = () => {
    if (filteredStudents.every((s: AcademyStudent) => selectedIds.includes(s.id))) setSelectedIds([])
    else setSelectedIds(filteredStudents.map((s: AcademyStudent) => s.id))
  }
  const handleSave = async () => {
    try {
      ...
      onClose()
    } catch (e: any) { alert('배정 실패: ' + e.message) }
  }
    ...
    <div className="fixed inset-0 z-[400] flex items-center justify-center"
      ...
      <div className="bg-white rounded-2xl flex flex-col overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.3)] w-[520px] max-h-[90vh]"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {loadingStudents || loadingAssignments ? (
              ...
              {filteredStudents.map(s => {
                ...
                return (
                  <label key={s.id}
                    ...
                    <input type="checkbox" checked={isSelected} onChange={() => toggleStudent(s.id)} className="w-4 h-4 accent-blue-600 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-[13px] font-bold text-ink">{s.name}</span>
                        <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full border"
                          style={{ background: gc.bg, color: gc.color, borderColor: gc.border }}>{s.grade}</span>
                      </div>
...
function MiddleQuestionTab() {
  ...
  const handleSave = async (data: any) => {
    try {
      if (editTarget) {
        const { level: _l, ...rest } = data
        await updateQ.mutateAsync({ id: editTarget.id, ...rest })
        alert(rest.is_draft ? '임시저장 되었어요!' : '문제가 수정되었어요!')
      } else {
        await createQ.mutateAsync(data)
        alert(data.is_draft ? '임시저장 되었어요!' : '문제가 만들어졌어요!')
      }
    ...
    <div>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2 pl-2">
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex gap-1">
            {(['전체', ...MIDDLE_TYPE_LIST] as const).map(t => (
              <button key={t} onClick={() => setFilterType(t)}
                className="px-2.5 py-1 rounded-full text-[11px] font-bold transition-all border"
                style={{ background: filterType === t ? MIDDLE_THEME.accent : '#fff', color: filterType === t ? '#fff' : '#6B7280', borderColor: filterType === t ? MIDDLE_THEME.accent : '#E5E7EB' }}>
                {t}
              </button>
            ))}
          </div>
      ...
      {isLoading ? (
        <div className="text-center py-16 text-ink-muted"><div className="text-[13px]">불러오는 중...</div></div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 bg-white border border-line rounded-2xl">
          <div className="text-[14px] font-bold text-ink mb-2">{questions.length === 0 ? '아직 만든 문제가 없어요' : '해당 조건의 문제가 없어요'}</div>
          {questions.length === 0 && (
            <button onClick={() => { setEditTarget(null); setShowForm(true) }}
              className="px-5 py-2.5 text-white rounded-xl text-[13px] font-bold mt-2" style={{ background: MIDDLE_THEME.accent }}>
              첫 문제 만들기
            </button>
          )}
        </div>
        ...
        <div className="bg-white border border-line rounded-xl overflow-hidden">
          <div className="flex px-4 py-2.5 bg-gray-50 border-b border-line text-[11px] font-bold text-ink-muted">
            <div className="w-[22%]">제목</div><div className="w-[10%]">과목</div><div className="w-[10%]">유형</div>
            <div className="w-[8%]">학년</div><div className="w-[30%]">평가 요소</div><div className="w-[20%]"></div>
          </div>
          {filtered.map((q, idx) => {
              ...
              <div key={q.id}
                ...
                <div className="w-[22%] min-w-0 pr-3">
                  <div className="text-[13px] font-bold text-ink truncate flex items-center gap-1.5">
                    {q.title}
                    {q.is_draft && <span className="text-[10px] font-bold px-1.5 py-0.5 rounded border flex-shrink-0" style={{ background: '#FFF7ED', color: '#92400E', borderColor: '#FCD34D' }}>임시저장</span>}
                  </div>
                  <div className="text-[11px] text-ink-muted mt-0.5">{new Date(q.created_at).toLocaleDateString('ko-KR', { year: 'numeric', month: '2-digit', day: '2-digit' })}</div>
                </div>
                <div className="w-[10%] text-[12px] font-semibold text-ink-secondary">{q.subject}</div>
                <div className="w-[10%]">
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: MIDDLE_THEME.accentBg, color: MIDDLE_THEME.accentDark, border: `1px solid ${MIDDLE_THEME.accentBorder}60` }}>{q.type}</span>
                </div>
                <div className="w-[8%]">
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full border" style={{ background: gc.bg, color: gc.color, borderColor: gc.border }}>{q.grade}</span>
                </div>
                <div className="w-[30%] flex items-center gap-1 overflow-hidden">
                  {evalCriteria.length > 0 ? evalCriteria.map((e: EvalItem, i: number) => (
...
function HighQuestionFormModal({ initial, onClose, onSave, saving }: {
  initial?: HighSuhaengQuestion | null
  onClose: () => void
  onSave: (data: any) => void
  saving: boolean
}) {
  ...
  const [maxChars, setMaxChars] = useState(initial?.max_chars?.toString() || '')
  const [evalCriteria, setEvalCriteria] = useState<EvalItem[]>(
    (initial?.eval_criteria?.length ?? 0) > 0 ? (initial!.eval_criteria as EvalItem[]) : HIGH_DEFAULT_EVAL['탐구보고서']
  )
  const [aiLoading, setAiLoading] = useState(false)
  const isEdit = !!initial
  ...
  const handleTypeChange = (t: HighSuhaengType) => {
    setType(t)
    if (!isEdit) setEvalCriteria(HIGH_DEFAULT_EVAL[t])
  }
  ...
  const handleAiGenerate = async () => {
    ...
    setAiLoading(true)
    try {
      const { data, error } = await supabase.functions.invoke('ai-suhaeng-question', {
        body: { subject, type, grade, evalCriteria },
      })
      if (error) throw error
      if (!data?.success) throw new Error(data?.error || 'AI 생성 실패')
      const q = data.question
  ...
  const handleSave = () => {
    ...
    if (evalTotal !== 100) { alert(`평가 요소 합계가 ${evalTotal}점이에요.`); return }
    if (evalCriteria.some(e => !e.name.trim())) { alert('평가 항목명을 모두 입력해주세요.'); return }
    onSave({ type, grade, subject, title, content, min_chars: info.hasChars ? Number(minChars) : null, max_chars: info.hasChars ? Number(maxChars) : null, eval_criteria: evalCriteria, is_draft: false })
  }
    ...
    <div className="fixed inset-0 z-[400] flex items-center justify-center"
      ...
      <div className="bg-white rounded-2xl flex flex-col overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.3)] w-[660px] max-h-[94vh]"
        ...
        <div className="px-6 py-4 border-b border-line flex-shrink-0 flex items-center justify-between">
          <div>
            <div className="text-[16px] font-extrabold text-ink">{isEdit ? '문제 수정' : '새 문제 만들기'}</div>
            <div className="text-[12px] text-ink-secondary mt-0.5">고등 수행평가</div>
          </div>
          <button onClick={onClose} className="text-ink-muted hover:text-ink text-xl">×</button>
        </div>
        {!isEdit && (
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
          {mode === 'ai' && !isEdit && (
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-5">
              <div className="mb-4">
                <div className="text-[14px] font-extrabold text-ink">AI 문제 자동 생성</div>
                <div className="text-[11px] text-ink-secondary mt-0.5">과목·유형·학년 선택 후 AI가 문제를 만들어드려요</div>
              </div>
              <div className="mb-4">
                <label className="block text-[11px] font-bold text-ink mb-1.5">학년</label>
                <div className="flex gap-1.5">
                  {HIGH_GRADES.map(g => {
                    const gc = GRADE_COLOR[g]
                    return (
                      <button key={g} onClick={() => { setGrade(g); setSubject('') }}
                        className="flex-1 py-1.5 rounded-lg text-[11px] font-bold transition-all border"
                        style={{ background: grade === g ? gc.bg : '#fff', color: grade === g ? gc.color : '#9CA3AF', borderColor: grade === g ? gc.border : '#E5E7EB' }}>
                        {g}
                      </button>
              ...
              <div className="mb-3">
                <label className="block text-[11px] font-bold text-ink mb-1.5">과목</label>
                <div className="flex flex-wrap gap-1.5">
                  {(HIGH_SUBJECTS_BY_GRADE[grade] ?? HIGH_SUBJECTS_BY_GRADE["전체"]).map(s => (
                    <button key={s} onClick={() => setSubject(s)}
                      className="px-2.5 py-1 rounded-full text-[11px] font-bold transition-all border"
                      style={{ background: subject === s ? HIGH_THEME.accent : '#fff', color: subject === s ? '#fff' : '#6B7280', borderColor: subject === s ? HIGH_THEME.accent : '#E5E7EB' }}>
                      {s}
                    </button>
                  ))}
                </div>
              ...
              <div className="mb-3">
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {HIGH_TYPE_LIST.map(t => (
                    <button key={t} onClick={() => handleTypeChange(t)}
                      className="py-2 rounded-xl text-center transition-all border"
                      style={{ background: type === t ? HIGH_THEME.accent : '#fff', color: type === t ? '#fff' : '#374151', borderColor: type === t ? HIGH_THEME.accent : '#E5E7EB' }}>
                      <span className="text-[11px] font-bold">{t}</span>
                    </button>
                  ))}
                </div>
              ...
              <button onClick={handleAiGenerate} disabled={aiLoading || !subject}
                className="w-full py-3 text-white rounded-xl text-[13px] font-bold transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                style={{ background: HIGH_THEME.accent, boxShadow: `0 4px 12px ${HIGH_THEME.accentShadow}` }}>
                {aiLoading ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />AI가 문제를 만드는 중...</>) : 'AI로 문제 생성하기'}
              </button>
            </div>
          )}
          {(mode === 'direct' || isEdit) && (
            <>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {HIGH_TYPE_LIST.map(t => (
                    <button key={t} onClick={() => handleTypeChange(t)}
                      ...
                      style={{ background: type === t ? HIGH_THEME.accent : '#fff', color: type === t ? '#fff' : '#374151', borderColor: type === t ? HIGH_THEME.accent : '#E5E7EB', boxShadow: type === t ? `0 4px 12px ${HIGH_THEME.accentShadow}` : 'none' }}>
                      <span className="text-[11px] font-bold">{t}</span>
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-[12px] font-bold text-ink mb-2">문제 제목 <span className="text-red-500">*</span></label>
                <input type="text" value={title} onChange={e => setTitle(e.target.value)}
                  placeholder="예: 기후 변화와 에너지 전환에 대한 탐구 보고서"
                  className="w-full px-3 py-2 border border-line rounded-lg text-[13px] focus:outline-none" />
              </div>
              <div>
                <label className="block text-[12px] font-bold text-ink mb-2">문제 내용 <span className="text-red-500">*</span></label>
        ...
        <div className="px-6 py-4 border-t border-line flex-shrink-0 flex justify-end gap-2">
          ...
          {(mode === 'direct' || isEdit || title) && (
            <>
              <button onClick={() => onSave({ type, grade, subject, title, content, min_chars: info.hasChars ? Number(minChars) : null, max_chars: info.hasChars ? Number(maxChars) : null, eval_criteria: evalCriteria, is_draft: true })}
                disabled={saving || !title.trim()}
                className="px-4 py-2 rounded-lg text-[12px] font-bold transition-all disabled:opacity-60 border"
                style={{ background: '#FFF7ED', color: '#92400E', borderColor: '#FCD34D' }}>
                {saving ? '저장 중...' : '임시저장'}
              ...
              <button onClick={handleSave} disabled={saving}
                className="px-5 py-2 text-white rounded-lg text-[12px] font-bold transition-all disabled:opacity-60"
                style={{ background: HIGH_THEME.accent, boxShadow: `0 2px 8px ${HIGH_THEME.accentShadow}` }}>
                {saving ? '저장 중...' : isEdit ? '수정 완료' : '문제 저장'}
              </button>
            </>
          )}
...
function HighAssignModal({ question, onClose }: { question: HighSuhaengQuestion; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!loadingAssignments) setSelectedIds(assignments.map(a => a.student_id))
  }, [loadingAssignments, assignments.length])
  ...
  const toggleStudent = (id: string) =>
    setSelectedIds(prev => prev.includes(id) ? prev.filter((x: string) => x !== id) : [...prev, id])
  const toggleAll = () => {
    if (filteredStudents.every((s: HighAcademyStudent) => selectedIds.includes(s.id))) setSelectedIds([])
    else setSelectedIds(filteredStudents.map((s: HighAcademyStudent) => s.id))
  }
  const handleSave = async () => {
    try {
      await syncAssignments.mutateAsync({ question_id: question.id, student_ids: selectedIds })
      ...
      <div className="bg-white rounded-2xl flex flex-col overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.3)] w-[520px] max-h-[90vh]"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3">
          {loadingStudents || loadingAssignments ? (
            ...
            <div className="text-center py-10 text-ink-muted text-[12px]">
              {allStudents.length === 0 ? '등록된 학생이 없어요' : '해당 조건의 학생이 없어요'}
            </div>
          ) : (
            <div className="flex flex-col gap-1.5">
              {filteredStudents.map(s => {
...
function HighQuestionTab() {
  ...
  const createQ = useCreateHighSuhaengQuestion()
  const updateQ = useUpdateHighSuhaengQuestion()
  const deleteQ = useDeleteHighSuhaengQuestion()
  const [showForm, setShowForm] = useState(false)
  const [editTarget, setEditTarget] = useState<HighSuhaengQuestion | null>(null)
  const [assignTarget, setAssignTarget] = useState<HighSuhaengQuestion | null>(null)
  const [filterType, setFilterType] = useState<HighSuhaengType | '전체'>('전체')
  const [filterGrade, setFilterGrade] = useState('전체')
  ...
  const handleSave = async (data: any) => {
    try {
      if (editTarget) {
        ...
        alert(data.is_draft ? '임시저장 되었어요!' : '문제가 수정되었어요!')
      } else {
        await createQ.mutateAsync(data)
        alert(data.is_draft ? '임시저장 되었어요!' : '문제가 만들어졌어요!')
      }
  ...
  const handleDelete = async (q: HighSuhaengQuestion) => {
    ...
    if (!window.confirm(`"${q.title}" 문제를 삭제할까요?`)) return
    try {
      await supabase.from('high_suhaeng_question_assignments').delete().eq('question_id', q.id)
      await deleteQ.mutateAsync(q.id)
    } catch (e: any) { alert('삭제 실패: ' + e.message) }
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-4 flex-wrap gap-2 pl-2">
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex gap-1 flex-wrap">
            {(['전체', ...HIGH_TYPE_LIST] as const).map(t => (
              <button key={t} onClick={() => setFilterType(t)}
                className="px-2.5 py-1 rounded-full text-[11px] font-bold transition-all border"
                style={{ background: filterType === t ? HIGH_THEME.accent : '#fff', color: filterType === t ? '#fff' : '#6B7280', borderColor: filterType === t ? HIGH_THEME.accent : '#E5E7EB' }}>
                {t}
              </button>
            ))}
          </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-white border border-line rounded-2xl">
          ...
          {questions.length === 0 && (
            <button onClick={() => { setEditTarget(null); setShowForm(true) }}
              className="px-5 py-2.5 text-white rounded-xl text-[13px] font-bold mt-2" style={{ background: HIGH_THEME.accent }}>
              첫 문제 만들기
            </button>
          )}
        </div>
        ...
        <div className="bg-white border border-line rounded-xl overflow-hidden">
          <div className="flex px-4 py-2.5 bg-gray-50 border-b border-line text-[11px] font-bold text-ink-muted">
            <div className="w-[22%]">제목</div><div className="w-[10%]">과목</div><div className="w-[12%]">유형</div>
            <div className="w-[8%]">학년</div><div className="w-[28%]">평가 요소</div><div className="w-[20%]"></div>
          </div>
          {filtered.map((q, idx) => {
            const gc = GRADE_COLOR[q.grade] || GRADE_COLOR['전체']
            const evalCriteria = q.eval_criteria || []
            return (
              <div key={q.id}
                className={`flex px-4 py-3 items-center transition-colors hover:bg-gray-50 ${idx !== filtered.length - 1 ? 'border-b border-line' : ''}`}>
                <div className="w-[22%] min-w-0 pr-3">
                ...
                <div className="w-[10%] text-[12px] font-semibold text-ink-secondary">{q.subject}</div>
                <div className="w-[12%]">
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full" style={{ background: HIGH_THEME.accentBg, color: HIGH_THEME.accentDark, border: `1px solid ${HIGH_THEME.accentBorder}60` }}>{q.type}</span>
                </div>
                <div className="w-[8%]">
                  <span className="text-[11px] font-bold px-2 py-0.5 rounded-full border" style={{ background: gc.bg, color: gc.color, borderColor: gc.border }}>{q.grade}</span>
                </div>
                <div className="w-[28%] flex items-center gap-1 overflow-hidden">
                  {evalCriteria.length > 0 ? evalCriteria.map((e, i) => (
                    <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-ink-secondary whitespace-nowrap flex-shrink-0">
                      {e.name} <span className="font-bold">{e.score}점</span>
                    </span>
                  )) : <span className="text-[11px] text-ink-muted">-</span>}
