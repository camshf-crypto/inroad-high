// src/pages/admin/_pages/students/detail/_hooks/useMiddleSaenggibuQuestions.ts
// 🌱 중등 생기부 예상질문 어드민 hook (고등 hook의 중등 버전)
...
export function useStudentSaenggibuPdf(studentId: string, grade: number) {
  return useQuery({
    queryKey: ['admin-middle-saenggibu-pdf', studentId, grade],
    refetchInterval: 5000,
    refetchIntervalInBackground: false,
    queryFn: async () => {
      const { data: uploadData } = await supabase
        .from('middle_saenggibu_uploads')
        .select('*')
        .eq('student_id', studentId)
        .eq('grade', grade)
        .maybeSingle()

      // 🔥 중등 폴더: middle/{studentId}/
      const folderPath = `middle/${studentId}`
      ...
      return {
        saenggibu_pdf_url: uploadData?.pdf_path || `${folderPath}/${latestFile.name}`,
...
export function useDeleteStudentPdf() {
  ...
  return useMutation({
    ...
    onSuccess: ({ studentId, grade }) => {
      qc.invalidateQueries({ queryKey: ['admin-middle-saenggibu-pdf', studentId, grade] })
    },
  })
}
...
export function useStudentQuestions(studentId: string, grade: number) {
  return useQuery({
    queryKey: ['admin-middle-expect-questions', studentId, grade],
    refetchInterval: 3000,
    refetchIntervalInBackground: false,
    queryFn: async (): Promise<ExpectQuestion[]> => {
      const { data, error } = await supabase
        .from('middle_saenggibu_questions')
        .select('*')
        .eq('student_id', studentId)
        .eq('grade', grade)
...
export function useQuestionAnalyses(questionId: string | undefined) {
  return useQuery({
    queryKey: ['admin-middle-expect-analyses', questionId],
    refetchInterval: 3000,
    refetchIntervalInBackground: false,
    queryFn: async (): Promise<ExpectAnalysis[]> => {
      if (!questionId) return []
      const { data, error } = await supabase
        .from('middle_saenggibu_questions_analysis')
        .select('*')
...
export function useQuestionFollowups(questionId: string | undefined) {
  return useQuery({
    queryKey: ['admin-middle-expect-followups', questionId],
    queryFn: async (): Promise<ExpectFollowup[]> => {
      if (!questionId) return []
      const { data, error } = await supabase
        .from('middle_saenggibu_questions_followups')
        .select('*')
        .eq('question_id', questionId)
        .order('created_at', { ascending: true })
      if (error) throw error
...
// ─────────────────────────────────────────────
// 5. AI 예상질문 생성 (Edge Function 호출 - 고등과 공유)
// ─────────────────────────────────────────────
...
export function useGenerateAIQuestions() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!major_dept) {
        throw new Error('관심 학과 정보가 없어요. 학생이 업로드 시 학과를 선택해야 합니다.')
      }
      ...
      const { data: aiData, error: aiError } = await supabase.functions.invoke(
        'ai-generate-questions',
        {
          body: {
            ...
            schoolLevel: 'middle', // 🔥 중등 표시 (Edge Function 미지원 시 무시됨)
          },
        }
      )
    ...
    onSuccess: (_data, variables) => {
      qc.invalidateQueries({
        queryKey: ['admin-middle-expect-questions', variables.studentId, variables.grade]
      })
    },
  })
}
...
export function useGenerateAIAnalysis() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      qc.invalidateQueries({ queryKey: ['admin-middle-expect-analyses', variables.questionId] })
    },
  })
}
...
export function useAddManualQuestion() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      qc.invalidateQueries({ queryKey: ['admin-middle-expect-questions', variables.student_id, variables.grade] })
    },
  })
}
...
export function useToggleQuestionPublish() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin-middle-expect-questions'] })
    },
  })
}
...
export function usePublishAllQuestions() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      qc.invalidateQueries({ queryKey: ['admin-middle-expect-questions', variables.studentId, variables.grade] })
    },
  })
}
...
export function useSendFirstFeedback() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, v) => {
      qc.invalidateQueries({ queryKey: ['admin-middle-expect-analyses', v.questionId] })
    },
  })
}
...
export function useAddFollowup() {
  ...
  return useMutation({
    ...
    onSuccess: (_d, v) => {
      qc.invalidateQueries({ queryKey: ['admin-middle-expect-followups', v.question_id] })
    },
  })
}
...
export function useDeleteFollowup() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['admin-middle-expect-followups'] })
    },
  })
}
